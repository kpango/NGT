# ArcFlare P0 PRISM — Injected SearchContext / SearchParameters Implementation Plan
...
- Worktree root: `/home/kpango/go/src/github.com/kpango/NGT/.claude/worktrees/arcflare-beat-qbg` (branch `worktree-arcflare-beat-qbg` = `origin/feat/arcflare-speedup`).
...
```bash
IDX=/home/kpango/go/src/github.com/kpango/NGT/data/indices/arcflare_sift-128-euclidean
Q=/home/kpango/go/src/github.com/kpango/NGT/data/ann-benchmarks/sift-128-euclidean.hdf5
GOLDEN=/tmp/p0_golden.bin
```
- **Index load API (verbatim from `ann_bench.cpp:62-63`, two-step):** `auto idx = ArcFlare::ArcFlareIndex::load(std::string(IDX)+"/aqindex"); idx.loadV2(IDX);`
- **Namespace is `ArcFlare`** (top-level, confirmed `ArcFlareIndex.h:23`; NOT `NGT::ArcFlare`). `SearchResult { uint32_t id; float distance; float bq_distance; }` is in `DABSSearcher.h:32`. `dEff()`/`loadV2`/static `load` at `ArcFlareIndex.h:121/117/96`.
- **Queries are hdf5** (`tests/arcflare/hdf5_io.h`: `H5FloatDataset h5_read_float(path,"test")`), zero-padded to `idx.dEff()` (mirror `ann_bench.cpp:65-72`). The repo has NO fvecs path — do not use fvecs.
- **Other prebuilt indices** (cross-metric spot-checks): `arcflare_gist-960-euclidean`, `arcflare_glove-100-angular`, `arcflare_nytimes-256-angular`, `arcflare_fashion-mnist-784-euclidean` under `.../data/indices/`, with matching hdf5 under `.../data/ann-benchmarks/`.

**Runner recipe (REQUIRED for every run — verified working in Task 1).** The test binary has no rpath; a stale `/usr/local/lib/libngt.so.2` shadows the freshly built lib and causes `symbol lookup error` for `ArcFlareIndex::load`/`loadV2` without this. BLAS/LAPACK also live under `/tmp/blas-local` (not system-wide):
```bash
...
# e.g.:  eval "$RUN ./build_arcflare/tests/arcflare/test_p0_prism $IDX $Q $GOLDEN --verify"
```
- **New test targets** go into the `if(HDF5_FOUND)` → `foreach(HDF5_SRC ...)` loop in `tests/arcflare/CMakeLists.txt` (where `ann_bench` is), NOT a standalone `add_executable` — that loop wires `${HDF5_C_LIBRARIES}`/`${HDF5_INCLUDE_DIRS}`/`ngt`/`cxx_std_23`. (Diagnostics from the editor's clangd showing "ArcFlareIndex.h not found" / `std::is_same` errors are false positives — clangd lacks the project `-Ilib` + HDF5 flags; the CMake build is the source of truth.)

---

## File Structure

- **Create:** `lib/NGT/ArcFlare/SearchContext.h` — `SearchParameters`, `SearchContext`, `SearchContextPool`, `SearchContextGuard` (RAII).
- **Modify:** `lib/NGT/ArcFlare/ArcFlareIndex.h` — new `searchV2` overload, `IndexRuntimeConfig` member, `SearchContextPool` member, remove 4 setters, add `#include "SearchContext.h"`.
- **Modify:** `lib/NGT/ArcFlare/ArcFlareIndex.cpp` — resolve env in ctor; migrate `searchV2` (+ helpers `buildGlobalLUT`/`buildGlobalLUT16`) from `thread_local`/`aq_cfg()` to `ctx`/`params`; update internal callers (`rebuildGraphSelf` @2894, `searchBatch`).
- **Create:** `tests/arcflare/test_p0_prism.cpp` — golden recall byte-identical test + concurrent-safety (TSan) test.
- **Modify:** `tests/arcflare/CMakeLists.txt` (or the `tests/arcflare/Makefile` target list) — add `test_p0_prism` target.
...
**Files:**
- Create: `tests/arcflare/test_p0_prism.cpp`
- Create (build target): add `test_p0_prism` to `tests/arcflare/CMakeLists.txt`
...
// Two-step load (same as ann_bench.cpp): ArcFlareIndex::load(dir+"/aqindex") + loadV2(dir).
...
Mirror the existing `ann_bench` target in `tests/arcflare/CMakeLists.txt` (it already pulls in `hdf5_io.h` and links HDF5 — copy its exact link/include vars). Add:
```cmake
...
Run: `cmake --build build_arcflare --target test_p0_prism -j`
Expected: links cleanly. (If the build dir is stale, reconfigure: `cmake -S . -B build_arcflare` first. Build only this target — never `all`.)

- [ ] **Step 4: Capture the golden file against the prebuilt SIFT index**

Run: `OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 ./build_arcflare/tests/arcflare/test_p0_prism $IDX $Q $GOLDEN --capture`
Expected: `captured 2000 tuples` (200 queries × k=10; fewer if any query returns <k).

- [ ] **Step 5: Verify capture is self-consistent**

Run: `OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 ./build_arcflare/tests/arcflare/test_p0_prism $IDX $Q $GOLDEN --verify`
Expected: `PASS byte-identical (2000 tuples)`.

- [ ] **Step 6: Commit**

```bash
git add tests/arcflare/test_p0_prism.cpp tests/arcflare/CMakeLists.txt
git commit -m "test(arcflare): P0 golden byte-identical recall capture for searchV2"
```
...
**Files:**
- Create: `lib/NGT/ArcFlare/SearchContext.h`
...
```cpp
// lib/NGT/ArcFlare/SearchContext.h
#pragma once
#include <vector>
#include <array>
#include <atomic>
...
// NbrCand, SeedScore, AQLinearPool are hoisted from ArcFlareIndex.cpp into this header
// in Step 0 below, so SearchContext owns them and the header is self-contained.

namespace ArcFlare {   // top-level namespace, confirmed ArcFlareIndex.h:23
...
Move `AQLinearPool` (`ArcFlareIndex.cpp:134-177`) and the `NbrCand`/`SeedScore` struct definitions (grep them in ArcFlareIndex.cpp) out of ArcFlareIndex.cpp into `SearchContext.h` (above `struct SearchContext`). Add `#include "SearchContext.h"` near the top of `ArcFlareIndex.cpp` (after the existing ArcFlare includes, before first use) so ArcFlareIndex.cpp still sees them. This makes the header self-contained. (Task 5 Step 1 then becomes a no-op verification.)

- [ ] **Step 2: Compile-check via the existing golden test target**

Because ArcFlareIndex.cpp now `#include`s SearchContext.h, building `test_p0_prism` compiles the new header (the structs are defined but not yet used by searchV2, so recall is unchanged).
Run: `cmake --build build_arcflare --target test_p0_prism -j`
Then golden verify (Task 2 changes no logic → must stay byte-identical):
`eval "$RUN ./build_arcflare/tests/arcflare/test_p0_prism $IDX $Q $GOLDEN --verify"`
Expected: clean build + `PASS byte-identical (2000 tuples)`.

- [ ] **Step 3: Commit**

```bash
git add lib/NGT/ArcFlare/SearchContext.h
git commit -m "feat(arcflare): add SearchParameters/SearchContext/SearchContextPool (P0 PRISM scaffolding)"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.h` (add member + struct)
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.cpp:77-118` (AQConfig), constructor, `:3283` (`AQ_PACKED`)
...
- [ ] **Step 1: Add `IndexRuntimeConfig` to ArcFlareIndex.h**

```cpp
// ArcFlareIndex.h — near the other config structs
...
```
Add member (near `ArcFlareIndex.h:222` where `prop_` lives): `IndexRuntimeConfig rt_cfg_;`

- [ ] **Step 2: Populate `rt_cfg_` in the constructor from env (move the reads out of `aq_cfg()`)**

In the `ArcFlareIndex` constructor, read the 6 env vars once and assign to `rt_cfg_` (same defaults/parse as the current `AQConfig` struct at `ArcFlareIndex.cpp:77-116`). Keep `aq_cfg()` temporarily for the per-query tunables not yet migrated (removed in Task 6).

- [ ] **Step 3: Re-run golden verify (no behavior change expected)**

Run: `OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 ./build_arcflare/tests/arcflare/test_p0_prism <idx> <q> /tmp/p0_golden.bin --verify`
Expected: `PASS byte-identical`.

- [ ] **Step 4: Commit**

```bash
git add lib/NGT/ArcFlare/ArcFlareIndex.h lib/NGT/ArcFlare/ArcFlareIndex.cpp
git commit -m "refactor(arcflare): resolve structural env flags into immutable IndexRuntimeConfig member"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.h:110-113`
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.cpp:1491`
...
- [ ] **Step 1: Declare the new overload in ArcFlareIndex.h**

```cpp
// ArcFlareIndex.h — alongside existing searchV2 decl
...
```
Add members near `ArcFlareIndex.h:234`: `mutable SearchContextPool ctx_pool_;`

- [ ] **Step 2: Implement the delegating legacy body**

```cpp
// ArcFlareIndex.cpp — replace the current searchV2(query,k,...) entry; body moves to the new overload in Task 5
...
Run: `cmake --build build_arcflare --target test_p0_prism -j && OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 ./build_arcflare/tests/arcflare/test_p0_prism <idx> <q> /tmp/p0_golden.bin --verify`
Expected: `PASS byte-identical`.
...
```bash
git add lib/NGT/ArcFlare/ArcFlareIndex.h lib/NGT/ArcFlare/ArcFlareIndex.cpp
git commit -m "refactor(arcflare): add injected searchV2(query,params,ctx) overload; legacy delegates"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.cpp` (searchV2 body `1491-2724`, helpers `buildGlobalLUT`/`buildGlobalLUT16`)
- Modify: `lib/NGT/ArcFlare/SearchContext.h` (hoist `NbrCand`, `SeedScore`, `AQLinearPool` definitions here from ArcFlareIndex.cpp so the header is self-contained)
...
Move their definitions (`AQLinearPool` at `ArcFlareIndex.cpp:134-177`; locate `NbrCand`/`SeedScore` structs in ArcFlareIndex.cpp) into `SearchContext.h` above `struct SearchContext`. Leave a `using` alias or just rely on the namespace. Confirm ArcFlareIndex.cpp still sees them via the include.
...
```bash
git add lib/NGT/ArcFlare/ArcFlareIndex.cpp lib/NGT/ArcFlare/SearchContext.h
git commit -m "refactor(arcflare): migrate 25 searchV2 thread_local scratch buffers to injected SearchContext"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.cpp` (every `aq_cfg()` use inside `searchV2`; `AQConfig`/`aq_cfg()` at `77-118`)
...
Grep: `grep -n "aq_cfg()" lib/NGT/ArcFlare/ArcFlareIndex.cpp`. If only build-path uses remain (e.g. the `AQ_*` build flags at 898-1214 are separate locals — leave those), delete the struct + singleton at `77-118`. Otherwise leave and note remaining users.
...
```bash
git add lib/NGT/ArcFlare/ArcFlareIndex.cpp
git commit -m "refactor(arcflare): route searchV2 tunables through SearchParameters/IndexRuntimeConfig; drop aq_cfg singleton"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.h:61-76` (remove setters)
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.cpp:2894` (`rebuildGraphSelf`), `searchBatch`, any external test callers
...
```bash
git add lib/NGT/ArcFlare/ArcFlareIndex.h lib/NGT/ArcFlare/ArcFlareIndex.cpp
git commit -m "refactor(arcflare): remove non-thread-safe search setters; callers inject SearchParameters"
```
...
```bash
git add lib/NGT/ArcFlare/ArcFlareIndex.cpp
git commit -m "refactor(arcflare): public search() acquires SearchContext from pool (concurrent-reuse-ready)"
```
...
**Files:**
- Modify: `tests/arcflare/test_p0_prism.cpp` (add a `--concurrent` mode)

Proves the redesign goal: many threads sharing one `ArcFlareIndex`, each with its own pooled `SearchContext`, race-free. (No writers in P0 — the shared_lock already permits concurrent readers; this verifies the state migration removed the per-thread aliasing hazards.)
...
Run: `cmake --build build_arcflare --target test_p0_prism -j` then a TSan build:
`clang++ -std=c++2c -fsanitize=thread -O1 -g -I lib tests/arcflare/test_p0_prism.cpp -o /tmp/test_p0_tsan <link ngt objects>`
...
```bash
git add tests/arcflare/test_p0_prism.cpp
git commit -m "test(arcflare): P0 concurrent 16-thread shared-index race-free verification (TSan)"
```
...
Run:
```
grep -n "getenv\|thread_local\|aq_cfg()" lib/NGT/ArcFlare/ArcFlareIndex.cpp
```
...
git commit -m "build(arcflare): bump to C++26 (-std=c++2c) after PRISM search-path statelessness"
```
