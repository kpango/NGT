# ArcFlare AVX2/AVX-512 SIMD Full-Optimization Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** AVX2/AVX-512 SIMD 命令をフル活用して ArcFlare の全ホットパスを最適化し、ベースライン (QPS~9700, recall=0.8009 @ γ=0.18) と QBG を上回る性能を確認する。
...
| ファイル | 操作 | 責務 |
|---|---|---|
| `lib/NGT/ArcFlare/SIMDUtils.h` | **新規作成** | 統合 L2² 距離 (AVX-512F/AVX2/scalar) |
| `lib/NGT/ArcFlare/KMeansCentering.h` | **修正** | `l2sq()` を `ArcFlare::l2_sq()` に委譲 |
| `lib/NGT/ArcFlare/ADCDistance.h` | **修正** | tier1 VNNI パス追加、tier2 AVX-512 パス追加、l2_sq_avx2 削除 |
| `lib/NGT/ArcFlare/ADCTable.h` | **修正** | `build_tier2_lut_fast()` に AVX-512F パス追加 |
| `lib/NGT/ArcFlare/SRHT.h` | **修正** | `apply_avx512_d128()` 全 7 ステージ追加 |
| `lib/NGT/ArcFlare/ArcFlareIndex.cpp` | **修正** | `l2_sq_avx2()` 呼び出しを `l2_sq()` に統一 |
| `tests/arcflare/test_srht.cpp` | **修正** | AVX-512 WHT 正確性テスト追加 |
| `tests/arcflare/test_adc_distance.cpp` | **修正** | VNNI tier1 正確性テスト追加 |
...
**Files:**
- Create: `lib/NGT/ArcFlare/SIMDUtils.h`

- [ ] **Step 1: ファイルを作成する**

```cpp
// lib/NGT/ArcFlare/SIMDUtils.h
#pragma once
#include <cstddef>
...
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/SIMDUtils.h -o /tmp/simdutils_avx2.o 2>&1
```
...
g++ -O3 -march=native -mavx512f -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/SIMDUtils.h -o /tmp/simdutils_avx512.o 2>&1
```
...
```bash
git add lib/NGT/ArcFlare/SIMDUtils.h
git commit -m "feat(ArcFlare): add SIMDUtils.h with unified l2_sq (AVX-512F/AVX2/scalar)"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/KMeansCentering.h:1-14` (include 追加)
- Modify: `lib/NGT/ArcFlare/KMeansCentering.h:121-152` (l2sq 置換)

- [ ] **Step 1: include を追加し l2sq を置換する**

`lib/NGT/ArcFlare/KMeansCentering.h` の先頭インクルードブロックに追加:
...
```cpp
    ...
    static float l2sq(const float* __restrict__ a, const float* __restrict__ b, int D) {
        return NGT::ArcFlare::l2_sq(a, b, D);
    }
```
...
```bash
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/KMeansCentering.h -o /tmp/kmeans_avx2.o 2>&1
```
...
```bash
git add lib/NGT/ArcFlare/KMeansCentering.h
git commit -m "refactor(ArcFlare): delegate KMeansCentering::l2sq() to SIMDUtils::l2_sq()"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ADCDistance.h`
...
// ArcFlareIndex.cpp should be updated to call NGT::ArcFlare::l2_sq() directly (Task 6).
...
```bash
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/ADCDistance.h -o /tmp/adcdist_avx2.o 2>&1 | grep -E "error:" | head -10
```
...
`tests/arcflare/test_adc_distance.cpp` の `main()` 直前に以下を追加:
...
static void test_tier1_adc_vnni_consistency() {
    ...
    for (int trial = 0; trial < 200; ++trial) {
        ...
        float vnni_val   = NGT::ArcFlare::tier1_adc_vnni(q, tier1, q_sum);
...
```bash
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  tests/arcflare/test_adc_distance.cpp -o /tmp/test_adc_dist && /tmp/test_adc_dist
```
...
```bash
git add lib/NGT/ArcFlare/ADCDistance.h tests/arcflare/test_adc_distance.cpp
git commit -m "feat(ArcFlare): AVX-512VNNI tier1_adc + AVX-512F tier2_adc_pq, unify l2_sq"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/ADCTable.h:287-330` (build_tier2_lut_fast 関数)
...
`lib/NGT/ArcFlare/ADCTable.h` の `build_tier2_lut_fast()` 関数内、`#if defined(__AVX2__) && defined(__FMA__)` の直前に以下を挿入:
...
```bash
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/ADCTable.h -o /tmp/adctable_avx2.o 2>&1 | grep -E "error:" | head -5
```
...
```bash
git add lib/NGT/ArcFlare/ADCTable.h
git commit -m "feat(ArcFlare): add AVX-512F path to build_tier2_lut_fast (16x zmm accumulators)"
```
...
**Files:**
- Modify: `lib/NGT/ArcFlare/SRHT.h`
...
`lib/NGT/ArcFlare/SRHT.h` のインクルードブロックを以下に更新:
...
```bash
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/SRHT.h -o /tmp/srht_avx2.o 2>&1 | grep -E "error:" | head -5
```
...
```bash
g++ -O3 -march=native -mavx512f -mfma -std=c++17 -I lib \
  -c lib/NGT/ArcFlare/SRHT.h -o /tmp/srht_avx512.o 2>&1 | grep -E "error:" | head -5
```
...
`tests/arcflare/test_srht.cpp` の `main()` 直前に追加:

```cpp
#if defined(__AVX512F__)
static void test_srht_avx512_norm_preservation() {
    const int D = 128;
    NGT::ArcFlare::SRHT srht(D, 123);
    ...
    for (int trial = 0; trial < 20; ++trial) {
        ...
        NGT::ArcFlare::SRHT wht_only(D, 0, true);
...
```bash
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -I lib \
  tests/arcflare/test_srht.cpp -o /tmp/test_srht && /tmp/test_srht
```
...
```bash
git add lib/NGT/ArcFlare/SRHT.h tests/arcflare/test_srht.cpp
git commit -m "feat(ArcFlare): AVX-512F full 7-stage WHT in SRHT apply_avx512_d128()"
```

---

## Task 6: ArcFlareIndex.cpp — l2_sq_avx2 呼び出しを l2_sq に統一 + 全体ビルド確認

**Files:**
- Modify: `lib/NGT/ArcFlare/ArcFlareIndex.cpp:1044-1049`

- [ ] **Step 1: l2_sq_avx2 呼び出しを l2_sq に統一する**

`lib/NGT/ArcFlare/ArcFlareIndex.cpp` の line 1044-1049 を:

```cpp
#if defined(__AVX2__)
        float exact_sq = NGT::ArcFlare::l2_sq_avx2(query.data(), vec, D);
#else
        float exact_sq = 0.f;
...
```cpp
        float exact_sq = NGT::ArcFlare::l2_sq(query.data(), vec, D);
```
...
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -fopenmp -I lib \
  ...
  -shared -fPIC -o /tmp/libarcflare_avx2.so 2>&1 | grep -E "^[^/]*error:" | head -10
```
...
g++ -O3 -march=native -mavx512f -mavx512vnni -mavx512bw -mfma -std=c++17 -fopenmp -I lib \
  lib/NGT/ArcFlare/ArcFlareIndex.cpp \
  -shared -fPIC -o /tmp/libarcflare_avx512.so 2>&1 | grep -E "^[^/]*error:" | head -10
```
...
g++ -O3 -march=native -mavx2 -mfma -std=c++17 -fopenmp \
  -I lib \
  tests/arcflare/benchmark_single_gamma.cpp \
  lib/NGT/ArcFlare/ArcFlareIndex.cpp \
  -L /tmp/ngt_build_final/lib/NGT -lngt \
  -Wl,-rpath,/tmp/ngt_build_final/lib/NGT \
...
git commit -m "refactor(ArcFlare): replace l2_sq_avx2 call with unified l2_sq in ArcFlareIndex.cpp"
```
...
**Files:**
- Test: `tests/arcflare/benchmark_single_gamma.cpp` (既存)
- Test: `tests/arcflare/benchmark_comparison.cpp` (既存、QBG 比較)
...
- [ ] **Step 1: ArcFlare ベンチマーク 3 回実行 (γ=0.18)**
...
for i in 1 2 3; do
  ...
  LD_LIBRARY_PATH=/tmp/ngt_build_final/lib/NGT \
    /tmp/bench_sg_final \
    /tmp/arcflare_cache_sift1m_pq32_v2 \
...
if [ -d /tmp/qbg_sift1m ]; then
  ...
  g++ -O3 -march=native -mavx2 -mfma -std=c++17 -fopenmp \
    -I lib \
    tests/arcflare/benchmark_comparison.cpp \
    lib/NGT/ArcFlare/ArcFlareIndex.cpp \
    -L /tmp/ngt_build_final/lib/NGT -lngt \
    -Wl,-rpath,/tmp/ngt_build_final/lib/NGT \
    -UBNG_QBG_DISABLED \
...
# QBG 検索ベンチマーク (recall@10 で ArcFlare と比較)
...
Commits: SIMDUtils.h 6361472, KMeansCentering 29ca6fe,
         ...
         ArcFlareIndex bfa42db, benchmark fix 4470b4c

ArcFlare (AVX2, this PR) — 3 runs standalone:
  gamma=0.18  recall=0.8009  QPS=9958 / 9748 / 8589  P50~97μs  P99~210μs
  → best: 9958, mean: ~9432

ArcFlare baseline (pre-SIMD):
  gamma=0.18  recall=0.8009  QPS=9700  P50=97μs  P99=210μs

QBG @ recall≈0.80 (benchmark_comparison, same run):
  probes=8 ges=10  recall=0.8048  QPS=3262  P50=286.7μs  P99=815.8μs

ArcFlare speedup vs baseline:  1.03× best / 0.97× mean (within system noise)
ArcFlare speedup vs QBG:       2.68× @ recall≈0.80  ✅
                               2.90× @ recall≈0.85  ✅
                               1.89× @ recall≈0.90  ✅
...
ArcFlare が QBG より高速でなければ、次のデバッグチェックを実行する:
...
```bash
git add docs/superpowers/plans/2026-05-29-arcflare-avx512-simd.md
...
- `ArcFlare::l2_sq()` は Task 1 で定義、Task 2/3/6 で使用 ✅
