...
**Tech Stack:** C++26, HDF5 (via `tests/ngtaq/hdf5_io.h`), reuses `lib/NGT/NGTAQ/{SRHT,ADCDistance,PCAProjector,SoAGraph,VectorRecord}.h`. Dev machine is **AVX2-only** (no AVX-512/VNNI); VNNI kernels are compiled behind `#if defined(__AVX512VNNI__)` and the gate numbers (footprint/recall/hops) are path-independent.
...
2. **Footprint baseline = 66B/neighbor, NOT 104B.** The production batch path reads only the gpq4 block (`M*8+32` over 16 neighbors = 66B/neighbor at M=128); the 38B/node `VectorRecord` is **dead work on the batch path** (read only in the seed scan, AQIndex.cpp:2017,2210-2287). Compare RaBitQ per-neighbor vs **66B**. ⇒ SIFT 1-bit (20B) wins; **GIST 1-bit (132B) and 2-bit (262B) are LOSSES**; only `rabitq-pca` (D'≤224 → ≤32B) is a GIST win candidate.
3. **The beam MUST be production-faithful** (else it repeats the saturation mistake): rerank exactly **top-64 by the beam's own routing distance** (`refine_floor=64`, AQIndex.cpp:2563 — NOT k*100, NOT the whole pool); **no hop cap**; `ef = max(k_beam*2, max_visits)` (AQIndex.cpp:1721, `k_beam=(rerank_factor>1)?k*rerank_factor:k`); terminate on `has_next()==(cur<size && cur<ef)`; `term_eps`/`term_eps_fp16` gates OFF (default). Sweep `max_visits` to drive `ef` and build a recall-vs-hops curve.
...
## Build & run notes
- Build ONLY specific targets — **NEVER `all`** (`test_vector_record` broken). Mirror the `ann_bench` HDF5 target in `tests/ngtaq/CMakeLists.txt` (foreach at ~lines 62-80: `ngt ${HDF5_C_LIBRARIES}`, `${HDF5_INCLUDE_DIRS}`, `cxx_std_26`).
- **Runner recipe (REQUIRED):**
  ...
  SIFT_IDX=/home/kpango/go/src/github.com/kpango/NGT/data/indices/ngtaq_sift-128-euclidean
  GIST_IDX=/home/kpango/go/src/github.com/kpango/NGT/data/indices/ngtaq_gist-960-euclidean
...
## File Structure
- **Create:** `tests/ngtaq/rabitq_bench.cpp` (the tool: modes `--selftest --footprint --encode --pca-encode --diag-rank --e2e --gate`), `tests/ngtaq/RaBitQ.h` (encode/distance/kernels).
- **Modify:** `lib/NGT/NGTAQ/AQIndex.h` (ONE read-only accessor `graphForDiag()`), `tests/ngtaq/CMakeLists.txt` (add `rabitq_bench` to the HDF5 foreach).
...
**Files:** Modify `lib/NGT/NGTAQ/AQIndex.h`.

- [ ] **Step 1: Add the read-only accessor** near `rotateForDiag()`/`kmeansForDiag()` (~AQIndex.h:176), inside the public section:
```cpp
// Diagnostic-only: exposes the loaded graph for the standalone rabitq_bench. Read-only; does NOT touch searchV2.
const ::NGTAQ::SoAGraph* graphForDiag() const { return graph_.get(); }
```
- [ ] **Step 2: Build the ngt lib (NOT `all`)**: `cmake --build build_ngtaq --target ngt -j` → clean.
- [ ] **Step 3: Confirm the gpq4 baseline path.** The prebuilt indices are expected to report `hasGPQ4()==true` (ann_bench drives SIFT to 0.9950 via the batch path). The bench will assert `idx.hasGPQ4()` and use `idx.gpq4Dist(...)` (AQIndex.h:318) as the baseline distance; only self-train a codebook if `false`. (No runtime check needed in this task — Task 8 asserts it.)
- [ ] **Step 4: Commit**
```bash
git add lib/NGT/NGTAQ/AQIndex.h
git commit -m "feat(ngtaq): add read-only graphForDiag() accessor for the standalone rabitq_bench (P1)"
```

## Task 1: `RaBitQ.h` encode/distance + `--selftest`

**Files:** Create `tests/ngtaq/RaBitQ.h`; create `tests/ngtaq/rabitq_bench.cpp` (with `--selftest` mode).

- [ ] **Step 1: Implement `RaBitQ.h`** — the encode + distance per the design reference (1-bit + 2-bit WITH the `lo·Su` term). Use `NGT::NGTAQ::SRHT` (SRHT.h:21,37) for rotation and `float_to_fp16`/`fp16_to_float` (VectorRecord.h:29,68) for stored scalars. Provide a scalar reference `dot` first (kernels come in Task 2).
...
- [ ] **Step 3: Build + run** (after Task 10 wires CMake, or compile standalone): `cmake --build build_ngtaq --target rabitq_bench -j && eval "$RUN ./build_ngtaq/tests/ngtaq/rabitq_bench --selftest"` → all asserts pass.
- [ ] **Step 4: Commit** `test(ngtaq): RaBitQ.h encode/distance (1-bit + 2-bit lo·Su) + selftest`.
...
- [ ] **Step 4: Commit** `feat(ngtaq): RaBitQ int-dot kernels (scalar/AVX2 generic-D/guarded VNNI), path-equivalent`.
...
- [ ] **Step 3: Commit** `feat(ngtaq): rabitq_bench --footprint (per-neighbor, 66B gpq4 baseline)`.
...
- [ ] **Step 3: Commit** `feat(ngtaq): rabitq_bench --encode offline pipeline`.
...
- [ ] **Step 4: Commit** `feat(ngtaq): rabitq_bench PCA variant (whiten=false default, fp32-D' control)`.
...
- [ ] **Step 2: Run + Commit** `feat(ngtaq): rabitq_bench --diag-rank (non-gating)`.
...
- [ ] **Step 4: Commit** `feat(ngtaq): rabitq_bench --e2e production-faithful end-to-end smoke`.
...
- [ ] **Step 3: Commit** `feat(ngtaq): rabitq_bench --gate (un-gameable PROCEED/STOP on validated levers)`.

## Task 10: CMake wiring + dev smoke

- [ ] **Step 1:** Add `rabitq_bench` to `tests/ngtaq/CMakeLists.txt` HDF5 foreach (~62-80), mirroring `ann_bench`.
- [ ] **Step 2:** `cmake --build build_ngtaq --target rabitq_bench -j` clean; `eval "$RUN ./build_ngtaq/tests/ngtaq/rabitq_bench --footprint"` + `--selftest` green on dev (AVX2).
- [ ] **Step 3: Commit** `build(ngtaq): wire rabitq_bench into the HDF5 test targets`.
