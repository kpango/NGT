# NGTAQv2 — Beat QBG *and* QG by 2–10× QPS Across All ANN-Benchmarks Recall Levels

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make NGTAQv2 (AQ-DABS) deliver 2–10× the QPS of **both** QBG (Quantized Blob Graph) **and** QG (NGT-qg, the actual ANN-Benchmarks leaderboard entry) at every recall level {0.50, 0.70, 0.80, 0.85, 0.90, 0.95, 0.99} on all five datasets (SIFT-128, GIST-960, FashionMNIST-784 = L2; GloVe-100, NYTimes-256 = angular). 0.99 is targeted only where physically reachable.

**Architecture:** Five-phase campaign. (0) Migrate the exact-rerank store `raw_flat_` from fp32 to fp16. (1) Establish two credible competitor baselines: QG with **official ANN-Benchmarks params verbatim**, and QBG self-tuned via its documented CLI knobs. (2) Generate the full single- and multi-thread recall-QPS grid for NGTAQ, QBG, and QG; a cell PASSES only when NGTAQ ≥ 2× the *stronger* competitor. (3) Profile the angular search path. (4) Apply profile-/grid-driven optimizations. (5) Re-measure and emit an honest per-cell report scoped to passing cells.

**Tech Stack:** C++17 (`lib/NGT/NGTAQ/`), AVX2+F16C SIMD, HDF5 I/O, std::thread inter-query parallelism, NGT `ngt`/`qbg` CLI for QG, Python 3 for grid comparison.

---

## ⚠️ Worktree / base-ref note

This plan was authored in worktree `ngtaq-beat-qbg` (branch `worktree-ngtaq-beat-qbg`, based at commit `522a328`). **The in-progress uncommitted changes to `AQIndex.cpp/.h` (searchV2 `n_probe_override_`, `fixHoleTombstones`, `rebuild_max_seeds_`, fp16-prep in `half.hpp`) are NOT in this worktree** — they live in the main checkout's working tree on `feat/ngtaq-speedup`. **Execute this plan against that WIP state** (commit the WIP first, then branch, or run in the main checkout). Line numbers below reference the `feat/ngtaq-speedup` *working tree*; the executing engineer should confirm anchors by the surrounding code snippets (re-grep if a line moved).
...
| Q7 | Which baseline | **Both QBG and QG are co-primary.** A cell passes only if NGTAQ ≥ 2× **max(QBG, QG)** at that recall. |
...
|---------|--------|---|---|------|------|------|------|------|------|------|
| SIFT-128 | L2 | 1M | 128 | ? | ? | ? | ? | ? | ? | NGTAQ 6046 vs QBG 2406 (2.5×); QG 未測 |
| FashionMNIST-784 | L2 | 60K | 784 | ? | ? | ? | ? | ? | ? | NGTAQ 0.9999@3098; both 未測 |
| GIST-960 | L2 | 1M | 960 | — | — | — | — | — | — | — (all 未測) |
| GloVe-100 | angular | 1.18M | 100 | ? | ? | ? | ? | △ | △ | NGTAQ ~0.99@291 (weak) |
| NYTimes-256 | angular | 290K | 256 | NGTAQ ~45 QPS 全帯（壊滅） |||||||

`?` = unmeasured on current build. `—` = no data. Only SIFT-128 @ 0.99 vs QBG is demonstrated; **QG never measured against NGTAQ on any cell.**

## Known risks (each has a mitigation task)

- **R1 — L2 mid/low-recall ceiling:** QBG/QG are flat-fast (≈6–14k+ QPS) at recall 0.5–0.9; QG especially (pure quantized graph) raises the bar. NGTAQ's fixed overhead (SRHT, per-cluster seeding, `EXPAND_N=200`, `refine_n=k_beam*100`, exact rerank) may cap QPS below 2×. → **Phase 4B BQ-only fast path.**
...
| HDF5 | NGTAQ/QBG metric | QG CLI `-D` | NGTAQ edge_size |
|------|------------------|-------------|-----------------|
...
> All NGTAQ angular builds use `edge_size=100` (the prior `edge_size=10` is the README-blamed thin-graph cause).
...
**Files:** `lib/NGT/NGTAQ/SIMDUtils.h`, `lib/NGT/NGTAQ/AQIndex.h:150`, `lib/NGT/NGTAQ/AQIndex.cpp` (all `raw_flat_` sites), `tests/ngtaq/test_l2_fp16.cpp` (new).

### Task 0.1: Add F16C `l2_sq_f32_fp16` + unit test

- [ ] **Step 1: Failing test** — `tests/ngtaq/test_l2_fp16.cpp`

```cpp
// tests/ngtaq/test_l2_fp16.cpp — fp32-query vs fp16-stored squared-L2 distance.
#include "NGT/NGTAQ/SIMDUtils.h"
...
- [ ] **Step 2: Add target + run; expect compile failure.** Add `test_l2_fp16` to the non-HDF5 `foreach` in `tests/ngtaq/CMakeLists.txt` (same pattern as other `test_*`). Then:

```bash
cd "$BUILD" && cmake . >/dev/null && cmake --build . --target test_l2_fp16 2>&1 | tail -5
# Expected: error: 'l2_sq_f32_fp16' is not a member of 'NGT::NGTAQ'
```

- [ ] **Step 3: Implement.** Add `#include "NGT/NGTAQ/VectorRecord.h"` at the top of `SIMDUtils.h`, then append after `l2_sq` (ends line 64), inside `namespace NGT { namespace NGTAQ {`:
...
```bash
cd "$BUILD" && cmake --build . --target test_l2_fp16 2>&1 | tail -3 && ./tests/ngtaq/test_l2_fp16
...
```bash
cd "$NGT" && git add lib/NGT/NGTAQ/SIMDUtils.h tests/ngtaq/test_l2_fp16.cpp tests/ngtaq/CMakeLists.txt
git commit -m "feat(ngtaq): add l2_sq_f32_fp16 (F16C) for fp16 exact rerank
...
Sites (verify by snippet): decl `AQIndex.h:150`; ctor `:35`; v1 search `:173-194`; `insert` `:258-262`; `rebuild` reorder `:321-333`; `save`/`load` `:381-384`; **searchV2 rerank `:1293-1298`**; `fixHoleTombstones` `:1390-1400`; `loadV2`/`rebuildGraphSelf` `:1438-1473`.

- [ ] **Step 1:** `AQIndex.h:150` → `std::vector<uint16_t> raw_flat_;`. Change `rawFlat()` (`:108`) to `const uint16_t*`. Fix callers: `grep -rn "rawFlat()" "$NGT/tests" "$NGT/lib"` and convert each via `fp16_to_float`.
...
```cpp
    for (int d = 0; d < D; ++d) raw_flat_.push_back(NGT::NGTAQ::float_to_fp16(vec[d]));
    if (is_angular_) {
        uint16_t* h = raw_flat_.data() + static_cast<size_t>(new_id) * D;
        float n2=0.f; for(int d=0;d<D;++d){ float f=NGT::NGTAQ::fp16_to_float(h[d]); n2+=f*f; }
        if (n2>1e-12f){ float inv=1.f/std::sqrt(n2);
            for(int d=0;d<D;++d) h[d]=NGT::NGTAQ::float_to_fp16(NGT::NGTAQ::fp16_to_float(h[d])*inv); }
    }
```
...
```cpp
    ...
    for (int d = 0; d < D; ++d) v[d] = NGT::NGTAQ::fp16_to_float(h[d]);
```

- [ ] **Step 8: Build.** `cd "$BUILD" && cmake --build . --target ngt build_ngtaqv2_ann ann_bench -j"$(nproc)" 2>&1 | tail -8` (expect no errors).
- [ ] **Step 9: Commit.**

```bash
cd "$NGT" && git add lib/NGT/NGTAQ/AQIndex.h lib/NGT/NGTAQ/AQIndex.cpp
git commit -m "feat(ngtaq): store raw_flat_ as fp16; rerank via l2_sq_f32_fp16 (Q1)
...
```bash
cd "$BUILD/tests/ngtaq"
./build_ngtaqv2_ann "$DATA/sift-128-euclidean.hdf5" "$IDX/ngtaq_sift-128-euclidean" l2 2000 64 1.2 100 2>&1 | tail -4
./ann_bench "$IDX/ngtaq_sift-128-euclidean" "$DATA/sift-128-euclidean.hdf5" 10 0.20 0.40 3 1 0
```
...
## Phase 2 — Full grid: NGTAQ vs max(QBG, QG) (Q4, Q6, Q7)

### Task 2.1: Build NGTAQ (fp16) indices for all 5 datasets

- [ ] **Step 1:** `benchmarks/results/grid/build_ngtaq.sh`:
...
for spec in sift-128-euclidean:l2:100 gist-960-euclidean:l2:100 \
  ...
  echo "=== build NGTAQ $ds ($m, edge=$edge) $(date) ==="
  ./build_ngtaqv2_ann "$DATA/$ds.hdf5" "$IDX/ngtaq_$ds" "$m" 2000 64 1.2 "$edge" \
    2>&1 | tee "$LOGS/ngtaq_build_$ds.log"
done
```

- [ ] **Step 2:** Run in background; verify each log ends `[Done] Saved to:`.

### Task 2.2: Sweep NGTAQ recall-QPS (single + multi thread)

`ann_bench <idx> <hdf5> k gamma_enq gamma_term rerank_factor n_threads n_probe`.

- [ ] **Step 1:** `benchmarks/results/grid/run_ngtaq.sh`:

```bash
#!/usr/bin/env bash
set -euo pipefail
cd "$BUILD/tests/ngtaq"; NPROC="$(nproc)"
...
for ds in "${!METRIC[@]}"; do
  m="${METRIC[$ds]}"; ge="${GENQ[$m]}"; hdf5="$DATA/$ds.hdf5"; idx="$IDX/ngtaq_$ds"
  for T in 1 "$NPROC"; do
    log="$LOGS/ngtaq_${ds}_t${T}.log"; : > "$log"
...
- [ ] **Step 2:** Run in background; verify ≥13 result blocks per `ngtaq_${ds}_t1.log`.
...
def main():
    if len(sys.argv)<4: print("usage: compare_grid.py ngtaq qbg qg [label]"); sys.exit(1)
    ...
    for t in TARGETS:
        ...
        if a is None and t>ng_max+1e-9:
            print(f"{t:>7.2f} | {'unreach':>8} | {fmt(b):>8} | {fmt(c):>8} | {'—':>7} | NGTAQ cannot reach"); continue
...
for ds in sift-128-euclidean gist-960-euclidean fashion-mnist-784-euclidean \
  ...
  python3 compare_grid.py "ngtaq_${ds}_t1.log" "qbg_${ds}_t1.log" "qg_${ds}_t1.log" "$ds (1T)"
...
**Files:** `lib/NGT/NGTAQ/AQIndex.cpp` (`-DAQ_PROFILE`-gated timers in `searchV2`).
...
```bash
cd "$BUILD/tests/ngtaq"
./ann_bench "$IDX/ngtaq_nytimes-256-angular" "$DATA/nytimes-256-angular.hdf5" \
...
Root cause: angular scores **all** members of all `n_probe`(=20) clusters (`take = members.size()`, `AQIndex.cpp:1084`) with per-cluster LUT rebuilds.

- [ ] **Step 1:** `AQIndex.cpp:1084-1086`:
...
Root cause R1/R5: at low recall NGTAQ still pays full rerank+expand while QBG/QG run lean.
...
cd tests/ngtaq; : > "$LOGS/ngtaq_sift_bqonly_t1.log"
for gt in 0.02 0.05 0.10 0.15 0.20 0.30; do
  ./ann_bench "$IDX/ngtaq_sift-128-euclidean" "$DATA/sift-128-euclidean.hdf5" 10 0.20 "$gt" -1 1 0 \
    2>/dev/null | tee -a "$LOGS/ngtaq_sift_bqonly_t1.log"; done
cat "$LOGS/ngtaq_sift-128-euclidean_t1.log" "$LOGS/ngtaq_sift_bqonly_t1.log" > "$LOGS/ngtaq_sift_full_t1.log"
python3 "$LOGS/compare_grid.py" "$LOGS/ngtaq_sift_full_t1.log" \
...
- [ ] Re-run `run_ngtaq.sh` (optimized binary + BQ-only low band where it wins), `compare_grid.py` for all 5 datasets at threads=1 and `nproc`; save `grid_single_thread.txt`, `grid_multi_thread.txt`.

### Task 5.2: `benchmarks/results/grid/REPORT.md`

- [ ] **Step 1:** Per-cell verdict table (datasets × {0.50…0.99}): NGTAQ, QBG, QG QPS, ratio vs max, verdict. Single-thread primary; multi-thread alongside.
- [ ] **Step 2:** Honest claim: "NGTAQ ≥2× max(QBG,QG) on cells {…}; cells {…} are win<2× / unreached." Memory caveat (Q1): NGTAQ stores fp16 vectors (2 B/dim) + 38 B/record vs QBG PQ-only / QG. Record sizes:

```bash
cd "$IDX" && for d in ngtaq_* qbg_* qg_*; do printf "%-40s %s\n" "$d" "$(du -sh "$d"|cut -f1)"; done \
