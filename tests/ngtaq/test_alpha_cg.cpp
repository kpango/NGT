...
void testNoPruneWhenNoCandidates() {
    NGTAQ::AlphaCGPruner pruner(1.2f, 1.0f);
...
void testKeepAllWhenFarApart() {
    ...
    NGTAQ::AlphaCGPruner pruner(1.2f, 0.0f /* kappa=0: no tau adjustment */);
...
void testPruneRedundantCandidate() {
    ...
    NGTAQ::AlphaCGPruner pruner(1.2f, 0.0f);
...
void testTauMakesThresholdMoreConservative() {
    ...
    NGTAQ::AlphaCGPruner pruner(1.2f, 1.0f); // kappa=1
    float tau = 0.05f;
