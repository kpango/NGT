#!/usr/bin/env bash
# ci_bench.sh — fair single-thread recall→QPS sweep, NGTAQ vs QG, for one dataset.
#
# NGTAQ: ann_bench swept over AQ_EF (the recall-QPS knob — see AQIndex.cpp:1692;
...
#
# Output:
#   $LOGS/ngtaq_<ds>_t1.log   raw ann_bench blocks
#   $LOGS/qg_<ds>_t1.log      raw qg_bench blocks
#   $LOGS/grid_<ds>.txt       compare_grid.py per-recall-band NGTAQ/QG ratio table
...
[ -s "$ngtaq_idx/aqindex" ] || { echo "ERROR: NGTAQ index missing: $ngtaq_idx" >&2; exit 1; }
...
echo "::group::[${DS}] NGTAQ AQ_EF sweep (single-thread)"
...
for ef in $AQ_EF_GRID; do
  AQ_EF="$ef" "$BIN/ann_bench" "$ngtaq_idx" "$HDF5" 10 "$GE" 0.90 3 1 "$NP" 0 2>/dev/null >> "$NLOG" || true
done
echo "[${DS}] NGTAQ: $(grep -c agg_QPS "$NLOG") points"
echo "::endgroup::"
...
# N/A for QBG and computes the ratio against QG alone (NGTAQ/QG).
GRID="$LOGS/grid_${DS}.txt"
python3 "$CMP" "$NLOG" "$LOGS/__no_qbg__.log" "$QLOG" "$DS NGTAQ-vs-QG (1T, CI)" | tee "$GRID"

# ---- markdown summary ----
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
  {
    echo "### ${DS} (${METRIC}) — NGTAQ vs QG (single-thread)"
    echo ""
    echo "NGTAQ points: $(grep -c agg_QPS "$NLOG")  ·  QG points: $(grep -c agg_QPS "$QLOG")"
    echo ""
    echo '```'
    cat "$GRID"
    echo '```'
