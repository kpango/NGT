// tests/ngtaq/rabitq_bench.cpp
// Offline RaBitQ-style residual-quantizer microbench for NGTAQ.
//
...
using NGT::NGTAQ::KMeansCentering;
...
// per-node centroid_id (AQIndex.cpp:731-751). Each residual is then SMALL —
...
int runKtest() {
    ...
    for (int di = 0; di < 2; ++di) {
        ...
        for (int t = 0; t < trials; ++t) {
            ...
            // 1-bit
            int64_t s1 = NGTAQ::rabitq_dot1_scalar(q_int.data(), bits.data(), D);
            int64_t a1 = NGTAQ::rabitq_dot1_avx2_or_scalar(q_int.data(), bits.data(), D);
            if (a1 != s1) {
            ...
            int64_t v1 = NGTAQ::rabitq_dot1_vnni(q_int.data(), bits.data(), D);
            if (v1 != s1) {
            ...
            // 2-bit
            int64_t s2 = NGTAQ::rabitq_dot2_scalar(q_int.data(), levels.data(), D);
            int64_t a2 = NGTAQ::rabitq_dot2_avx2_or_scalar(q_int.data(), levels.data(), D);
            if (a2 != s2) {
...
#if defined(__AVX512VNNI__)
            int64_t v2 = NGTAQ::rabitq_dot2_vnni(q_int.data(), levels.data(), D);
            if (v2 != s2) {
...
int runEncode(const char* hdf5_path) {
    ...
    const uint32_t K = NGT::NGTAQ::select_k(static_cast<uint64_t>(N));
...
// driven by AQIndex.cpp:1720-1722) EXACTLY:
...
BeamResult beam_walk(const SoAGraph* g, uint32_t N,
    ...
    BeamResult res;
    const int ef = std::max(k_beam * 2, max_visits);   // AQIndex.cpp:1720-1722
...
// (AQIndex.h:321-327), using gpq4_ip_table for the per-query IP table
...
struct GPQ4Self {
    ...
    void train(const float* xr_all, int N, int Dim, int M_sub) {
        ...
        M = M_sub;
        D_sub = D / M;
        cb.assign(static_cast<size_t>(M) * NGT::NGTAQ::GPQ4_K * D_sub, 0.f);
        ...
        for (int s = 0; s < M; ++s) {
            ...
            NGT::NGTAQ::KMeansCentering km(NGT::NGTAQ::GPQ4_K, D_sub,
                                           0xA5A5u + static_cast<uint64_t>(s));
            km.train(sub.data(), static_cast<size_t>(N));
            for (int k = 0; k < NGT::NGTAQ::GPQ4_K; ++k)
                std::memcpy(cb.data() + (static_cast<size_t>(s) * NGT::NGTAQ::GPQ4_K + k) * D_sub,
                            km.centroid(static_cast<uint32_t>(k)),
                            static_cast<size_t>(D_sub) * sizeof(float));
        }
        cb_T.resize(cb.size());
        NGT::NGTAQ::build_tier2_codebook_T(cb.data(), M, NGT::NGTAQ::GPQ4_K, D_sub, cb_T.data());
        ...
        for (int t = 0; t < nT; ++t) {
            th.emplace_back([&, t]() {
                for (int i = t; i < N; i += nT) {
                    ...
                    for (int s = 0; s < M; ++s) {
                        const float* cbs = cb.data() + (size_t)s * NGT::NGTAQ::GPQ4_K * D_sub;
                        cd[s] = NGT::NGTAQ::gpq4_encode_sub(xr + (size_t)s * D_sub, cbs, D_sub, rns);
                    }
    ...
    float buildQueryIP(const float* q_rot, std::vector<float>& ip_out) const {
        ip_out.resize(static_cast<size_t>(M) * NGT::NGTAQ::GPQ4_K);
        NGT::NGTAQ::gpq4_ip_table(q_rot, M, cb_T.data(), D_sub, ip_out.data());
        float q_ns = 0.f;
        for (int d = 0; d < D; ++d) q_ns += q_rot[d] * q_rot[d];
        return q_ns;
    }

    // Replicates NGTAQIndex::gpq4Dist (AQIndex.h:321-327) exactly.
    inline float dist(uint32_t id, const float* ip, float q_ns) const {
        const uint8_t* cd = codes.data() + static_cast<size_t>(id) * M;
        float ipsum = 0.f;
        for (int s = 0; s < M; ++s) ipsum += ip[(size_t)s * NGT::NGTAQ::GPQ4_K + cd[s]];
...
static double churn_recall(NGTAQ::RaBitQGraph& g, const float* train, int q_dim,
...
int runChurnTest(const std::string& hdf5_path) {
    ...
    NGTAQ::RaBitQGraph g(q_dim, 12345);
    ...
    NGTAQ::RaBitQGraph gs(q_dim, 12345);
...
static int runBuildSave(const std::string& hdf5_path, int Ncap, bool angular,
    ...
    NGTAQ::RaBitQGraph g(q_dim, 12345, Rdeg, angular);
...
static int runBenchPureLoad(const std::string& idx_path, const std::string& hdf5_path) {
    auto te = h5_read_float(hdf5_path, "test");
    NGTAQ::RaBitQGraph g = NGTAQ::RaBitQGraph::load(idx_path);
