...
#
# The whole point of running the NGTAQ-vs-QG benchmark on a GitHub-hosted runner
# is to exercise the AVX-512/VNNI distance kernels, which the local dev box
# (AVX2-only Zen 2) cannot. The NGTAQ SIMD kernels are compile-time gated on
# __AVX512F__ / __AVX512DQ__ (lib/NGT/NGTAQ/SIMDUtils.h, ADCTable.h) and selected
...
# ISA flags we care about for the NGTAQ kernels.
...
if [ "$has_avx512" = yes ]; then
  emit "> ✅ Runner advertises AVX-512. Built with \`-march=native\`, the NGTAQ"
