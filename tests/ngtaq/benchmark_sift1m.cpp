// tests/ngtaq/benchmark_sift1m.cpp
// SIFT-1M benchmark: measures QPS, P50/P99 latency, and recall@k for NGTAQIndex.
//
// Usage:
//   ./benchmark_sift1m <ngt_path> <query.fvecs> <groundtruth.ivecs> [k=10] [gamma_term=0.35]
//
// Ground truth IDs in SIFT-1M ivecs are 0-indexed and match NGTAQ node IDs directly.
...
int main(int argc, char** argv) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0]
                  ...
                  << "  aq_cache_path: if given, load NGTAQ index from cache (or build+save on first run)\n";
        return 1;
    }
    ...
    // Optional NGTAQ index cache: load if exists, else build + save
    ...
    NGTAQ::NGTAQIndex aq = [&]() -> NGTAQ::NGTAQIndex {
        ...
        if (!aq_cache_path.empty()) {
            ...
            if (probe.good()) {
                probe.close();
                std::cout << "Loading cached NGTAQIndex from: " << aq_cache_path << " ...\n";
                std::cout.flush();
                try {
                    auto idx = NGTAQ::NGTAQIndex::load(aq_cache_path);
        ...
        std::cout << "Building NGTAQIndex from: " << ngt_path << " (this may take ~30 min for 1M vecs) ...\n";
        std::cout.flush();
    ...
    std::vector<std::vector<NGTAQ::SearchResult>> all_results(nq);
    ...
    // --- Compute recall@k ---
    // GT IDs are 0-indexed; NGTAQIndex also returns 0-based IDs (fromNGT converts 1-based NGT IDs).
