...
// Compares QBG (Quantized Blob Graph) vs ArcFlare (AQ-DABS) recall-QPS on SIFT-1M.
//
// Usage:
//   ./benchmark_comparison <ngt_path> <base.fvecs> <query.fvecs> <gt.ivecs>
//                          [k=10] [arcflare_cache] [qbg_index_dir=/tmp/qbg_bench]
//
// First run builds both indexes (QBG takes 15-60 min for 1M vecs).
// Subsequent runs reuse cached indexes.
// Delete arcflare_cache or qbg_index_dir to force rebuild.
//
// SIFT-1M ground truth IDs are 0-indexed.
// ArcFlare returns 0-indexed IDs. QBG returns 1-indexed IDs (subtract 1 before GT lookup).
...
// --------------------------------------------------------------------------
// ArcFlare recall-QPS sweep over gamma_term values
// --------------------------------------------------------------------------
static std::vector<BenchRow> sweepArcFlare(
    ArcFlare::ArcFlareIndex& aq,
    ...
    int k)
{
    ...
    ArcFlare::SearchParameters params;
    params.k = k;
    ArcFlare::SearchContext ctx;

    // Warmup (200 queries, default params)
    params.gamma_enq = 0.15f; params.gamma_term = 0.35f;
    for (size_t i = 0; i < std::min(nq, size_t(200)); ++i)
        aq.searchV2(queries[i], params, ctx);

    for (float gt_val : GAMMA_TERMS) {
        const float enq_val = gt_val * 0.43f;
        params.gamma_enq = enq_val; params.gamma_term = gt_val;
        ...
        for (size_t i = 0; i < nq; ++i) {
            ...
            auto res = aq.searchV2(queries[i], params, ctx);
        ...
        std::cout << "  ArcFlare gamma_term=" << std::fixed << std::setprecision(3) << gt_val
    ...
    int k)
{
    ...
    std::cout << "\n=== ArcFlare k_prime_factor sweep (gamma_term=0.50, k=" << k << ") ===\n";
    ...
    ArcFlare::SearchParameters params;
    params.k = k;
    params.gamma_enq = 0.50f * 0.43f;  // gamma_term=0.50
    params.gamma_term = 0.50f;
    ArcFlare::SearchContext ctx;

    for (float kpf : KPF_VALS) {
        params.k_prime_factor = kpf;
...
// --------------------------------------------------------------------------
// sweepArcFlare: gamma_term sweep for ArcFlare (ADC search)
// --------------------------------------------------------------------------
static std::vector<BenchRow> sweepArcFlare(
...
int main(int argc, char** argv) {
    if (argc < 5) {
        std::cerr
            ...
            << " [k=10] [arcflare_cache] [qbg_index_dir=/tmp/qbg_bench]\n"
            << "\n"
            << "  ngt_path:      NGT index directory (source for ArcFlare fromNGT())\n"
            ...
            << "  arcflare_cache:   Path to save/load ArcFlare index cache\n"
    ...
    const std::string arcflare_cache = (argc > 6) ? argv[6] : "";
#ifndef NGT_QBG_DISABLED
    ...
    ArcFlare::ArcFlareIndex aq = [&]() -> ArcFlare::ArcFlareIndex {
        if (!arcflare_cache.empty()) {
            std::ifstream probe(arcflare_cache);
            if (probe.good()) {
                probe.close();
                std::cout << "[ArcFlare] Loading cache: " << arcflare_cache << "\n"; std::cout.flush();
                try {
                    auto idx = ArcFlare::ArcFlareIndex::load(arcflare_cache + "/aqindex");
                    idx.loadV2(arcflare_cache);
                    std::cout << "[ArcFlare] Loaded (v2). size=" << idx.size() << "\n\n"; std::cout.flush();
                    return idx;
                } catch (const std::exception& e) {
                    std::cerr << "[ArcFlare] Cache load failed (" << e.what() << "), rebuilding...\n";
                }
            }
        }
        std::cout << "[ArcFlare] Building from NGT: " << ngt_path << " ...\n"; std::cout.flush();
        const auto t0 = std::chrono::steady_clock::now();
        auto idx = ArcFlare::ArcFlareIndex::fromNGT(ngt_path, prop);
        const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::steady_clock::now() - t0).count();
        std::cout << "[ArcFlare] Built in " << elapsed << "s. size=" << idx.size() << "\n";
        std::cout.flush();
        if (!arcflare_cache.empty()) {
            try {
                idx.save(arcflare_cache);
                std::cout << "[ArcFlare] Saved to cache: " << arcflare_cache << "\n";
                std::cout.flush();
            } catch (const std::exception& e) {
                std::cerr << "[ArcFlare] Save failed: " << e.what() << "\n";
            }
        }
        return idx;
    }();
    ...
    // ==================== ArcFlare ====================
    const std::string v2_cache = arcflare_cache.empty() ? std::string("") : arcflare_cache + "_v2";
    ArcFlare::ArcFlareIndex aq_v2 = [&]() -> ArcFlare::ArcFlareIndex {
        if (!v2_cache.empty()) {
            std::ifstream probe(v2_cache + "/v2_records.bin");
            if (probe.good()) {
                probe.close();
                std::cout << "[ArcFlare] Loading cache: " << v2_cache << "\n"; std::cout.flush();
                try {
                    auto idx = ArcFlare::ArcFlareIndex::load(v2_cache + "/aqindex");
                    idx.loadV2(v2_cache);
                    std::cout << "[ArcFlare] Loaded.\n"; std::cout.flush();
                    return idx;
                } catch (const std::exception& e) {
                    std::cerr << "[ArcFlare] Cache load failed (" << e.what() << "), rebuilding...\n";
                }
        ...
        std::cout << "[ArcFlare] Building from NGT: " << ngt_path << " ...\n"; std::cout.flush();
        ArcFlare::ArcFlareIndex::Property v2_prop;
        v2_prop.dimension = D;
        const auto t0 = std::chrono::steady_clock::now();
        auto idx = ArcFlare::ArcFlareIndex::fromNGTv2(ngt_path, v2_prop);
        const auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
            std::chrono::steady_clock::now() - t0).count();
        std::cout << "[ArcFlare] Built in " << elapsed << "s\n"; std::cout.flush();
        if (!v2_cache.empty()) {
            try {
                std::filesystem::create_directories(v2_cache);
                idx.save(v2_cache + "/aqindex");
                idx.saveV2(v2_cache);
                std::cout << "[ArcFlare] Saved to cache: " << v2_cache << "\n"; std::cout.flush();
            } catch (const std::exception& e) {
                std::cerr << "[ArcFlare] Save failed: " << e.what() << "\n";
            }
        }
    ...
    sweepArcFlareKprime(aq, queries, ground_truth, k);

    std::cout << "\n=== ArcFlare sweep (gamma_term) ===\n"; std::cout.flush();
    auto arcflare_rows = sweepArcFlare(aq, queries, ground_truth, k);

    std::cout << "\n=== ArcFlare sweep (gamma_term) ===\n"; std::cout.flush();
    auto v2_rows = sweepArcFlare(aq_v2, queries, ground_truth, k);
    ...
    std::cout << "\n\n";
    std::cout << "================================================================\n";
    std::cout << "  Comparison: ArcFlare (AQ-DABS BQ) vs QBG (Quantized Blob Graph)\n";
    std::cout << "  N=" << N << "  D=" << D << "  queries=" << nq << "  k=" << k << "\n";
    std::cout << "================================================================\n";

    printTable("--- ArcFlare (AQ-DABS Binary Quantization) ---", "gamma_term", arcflare_rows, k);
    printTable("--- ArcFlare (ADC Quantization) ---",          "gamma_term", v2_rows,    k);
#ifndef NGT_QBG_DISABLED
    ...
    for (double target_recall : {0.80, 0.85, 0.90, 0.95}) {
        ...
        const auto* nv   = nearest(arcflare_rows, target_recall);
        const auto* v2v  = nearest(v2_rows,    target_recall);
#ifndef NGT_QBG_DISABLED
        const auto* qv   = nearest(qbg_rows,   target_recall);
#endif
        if (nv)  std::cout << "  ArcFlare:   recall=" << std::setprecision(4) << nv->recall  << "  QPS=" << std::setprecision(0) << nv->qps  << "  P50=" << std::setprecision(1) << nv->p50_us  << "us  P99=" << nv->p99_us  << "us\n";
        if (v2v) std::cout << "  ArcFlare: recall=" << std::setprecision(4) << v2v->recall << "  QPS=" << std::setprecision(0) << v2v->qps << "  P50=" << std::setprecision(1) << v2v->p50_us << "us  P99=" << v2v->p99_us << "us\n";
#ifndef NGT_QBG_DISABLED
        ...
        if (v2v && qv && qv->qps > 0) {
            ...
            std::cout << "  ArcFlare vs QBG: ";
            if (ratio > 1.05) std::cout << "ArcFlare " << std::setprecision(2) << ratio << "x faster\n";
    ...
    const auto* n90  = nearest(arcflare_rows, 0.90);
    const auto* v290 = nearest(v2_rows,    0.90);
    if (n90) {
        std::cout << "  ArcFlare (gamma_term=" << std::setprecision(3) << n90->param_val << "):"
    ...
    if (v290) {
        std::cout << "  ArcFlare (gamma_term=" << std::setprecision(3) << v290->param_val << "):"
    ...
    if (n90 && v290 && v290->qps > 0 && n90->qps > 0) {
        ...
        std::cout << "  ArcFlare vs ArcFlare at recall~0.90: ";
        if (ratio > 1.05)
            std::cout << "ArcFlare is " << std::setprecision(2) << ratio << "x faster\n";
        else if (ratio < 0.95)
            std::cout << "ArcFlare is " << std::setprecision(2) << (1.0/ratio) << "x faster\n";
        else
    ...
    if (n90 && q90 && q90->qps > 0) {
        ...
        std::cout << "  ArcFlare vs QBG at recall~0.90: ";
        if (ratio > 1.05)
            std::cout << "ArcFlare is " << std::setprecision(2) << ratio << "x faster\n";
        else if (ratio < 0.95)
    ...
    if (v290 && q90 && q90->qps > 0) {
        ...
        if (ratio > 1.05)
            ...
            std::cout << "QBG is " << std::setprecision(2) << (1.0/ratio) << "x faster than ArcFlare\n";
        else
            std::cout << "Roughly equivalent (" << std::setprecision(2) << ratio << "x)\n";
    }
#endif // NGT_QBG_DISABLED
