#include "../../lib/NGT/ArcFlare/PCAProjector.h"
#include <cstdio>
#include <cmath>
#include <vector>
#include <random>
...
static void test_output_dim() {
    ...
    NGT::ArcFlare::PCAProjector pca(D, TOP, 42);
...
static void test_variance_captured() {
    ...
    NGT::ArcFlare::PCAProjector pca(D, TOP, 0);
    pca.fit(data.data(), N);
...
static void test_whitened_unit_variance() {
    ...
    for (auto& v : data) v = nd(rng);
    NGT::ArcFlare::PCAProjector pca(D, TOP, 0, /*whiten=*/true);
    pca.fit(data.data(), N);
