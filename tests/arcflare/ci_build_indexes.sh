#!/usr/bin/env bash
# ci_build_indexes.sh — build the ArcFlare and QG indexes for one ANN-Benchmarks dataset.
#
# ArcFlare: M=128 (max_edges=128) + ONNG graph reconstruction (AQ_ONNG=1) + GORDER
...
#   <build_dir>  CMake build tree (contains tests/arcflare/<tools> and lib/NGT/libngt.so)
#   <data_dir>   dir holding <dataset>.hdf5
#   <idx_dir>    output dir for indexes (arcflare_<ds>, qg_<ds>_*)
...
echo "::group::[${DS}] ArcFlare index (M=128, ONNG, GORDER) build"
if [ -s "$arcflare_idx/aqindex" ]; then
  echo "[skip] ArcFlare index already present: $arcflare_idx"
else
  t0=$(date +%s)
  # build_arcflarev2_ann <hdf5> <out_dir> <metric> <k_clusters=0> <max_edges> <alpha> <edge_size>
  ...
  AQ_ONNG=1 AQ_ONNG_O="${AQ_ONNG_O:-32}" AQ_ONNG_I="${AQ_ONNG_I:-64}" \
    "$BIN/build_arcflarev2_ann" "$HDF5" "$arcflare_idx" "$METRIC" 0 128 1.2 10
  echo "[ok] ArcFlare index built in $(( $(date +%s) - t0 ))s -> $arcflare_idx"
fi
echo "::endgroup::"
...
du -sh "$arcflare_idx" "${qg_base}_qsg2" 2>/dev/null || true
