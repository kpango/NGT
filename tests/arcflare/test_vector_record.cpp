// tests/arcflare/test_vector_record.cpp
#include "../../lib/NGT/ArcFlare/VectorRecord.h"
#include <cassert>
#include <cmath>
#include <cstdint>
#include <cstdio>
...
static void test_fp16_roundtrip() {
    ...
    for (float v : values) {
        uint16_t h = NGT::ArcFlare::float_to_fp16(v);
        float back = NGT::ArcFlare::fp16_to_float(h);
...
static void test_fp16_special() {
    EXPECT_EQ(NGT::ArcFlare::float_to_fp16(0.0f), 0);
    EXPECT_NEAR(NGT::ArcFlare::fp16_to_float(0), 0.0f, 1e-6f);
}

static void test_vector_record_size() {
    EXPECT_EQ(sizeof(NGT::ArcFlare::VectorRecord), 38u);
}

static void test_vector_record_layout() {
    EXPECT_EQ(offsetof(NGT::ArcFlare::VectorRecord, tier1), 0u);
    EXPECT_EQ(offsetof(NGT::ArcFlare::VectorRecord, tier2), 16u);
    EXPECT_EQ(offsetof(NGT::ArcFlare::VectorRecord, norm_fp16), 32u);
    EXPECT_EQ(offsetof(NGT::ArcFlare::VectorRecord, centroid_id), 34u);
}

static void test_tier1_bit_set() {
    NGT::ArcFlare::VectorRecord rec = {};
    NGT::ArcFlare::set_tier1_bit(rec, 5, true);
    EXPECT_TRUE(NGT::ArcFlare::get_tier1_bit(rec, 5));
    NGT::ArcFlare::set_tier1_bit(rec, 5, false);
    EXPECT_TRUE(!NGT::ArcFlare::get_tier1_bit(rec, 5));
}
...
static void test_tier2_nibble() {
    ...
    for (int i = 0; i < 32; ++i) {
        ...
        NGT::ArcFlare::set_tier2_nibble(rec, i, val);
        EXPECT_EQ(NGT::ArcFlare::get_tier2_nibble(rec, i), val);
    }
}
