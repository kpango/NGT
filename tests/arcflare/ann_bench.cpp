// tests/arcflare/ann_bench.cpp
// Unified ANN-Benchmarks benchmark for ArcFlare.
...
int main(int argc, char** argv) {
    ...
    ArcFlare::ArcFlareIndex idx = ArcFlare::ArcFlareIndex::load(std::string(idx_dir) + "/aqindex");
    idx.loadV2(idx_dir);

    // Base per-query tuning, injected via the thread-safe SearchParameters/SearchContext
    // API. Each worker thread gets its own SearchContext (never shared).
    ArcFlare::SearchParameters params;
    params.k             = k;
    params.gamma_enq     = gamma_enq;
    params.gamma_term    = gamma_term;
    params.rerank_factor = rerank_factor;
    params.max_visits    = max_visits;
    if (n_probe > 0)            params.n_probe          = n_probe;
    if (seeds_per_cluster >= 0) params.seeds_per_cluster = seeds_per_cluster;
    ...
    {
        ArcFlare::SearchContext warm_ctx;
        for (int qi = 0; qi < WARMUP; ++qi)
            idx.searchV2(queries[qi % nq], params, warm_ctx);
    }
    ...
    for (int t = 0; t < n_threads; ++t) {
        threads.emplace_back([&]() {
            // One SearchContext per thread, declared outside the per-query loop so its
            // scratch buffers are reused across queries. Never shared between threads.
            ArcFlare::SearchContext ctx;
            for (;;) {
                int qi = next_qi.fetch_add(1, std::memory_order_relaxed);
                if (qi >= nq) break;
                const double t0 = now_us();
                auto sr = idx.searchV2(queries[qi], params, ctx);
