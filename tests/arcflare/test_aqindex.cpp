// tests/arcflare/test_aqindex.cpp
//
// Integration tests for ArcFlareIndex.
...
void testFromNGTAndSearch() {
    ...
    auto aq = ArcFlare::ArcFlareIndex::fromNGT(idx_path, prop);
    ...
    // and doesn't crash.
    {
        ArcFlare::ArcFlareIndex::Property p2;
        p2.dimension = D;
        p2.n_tau_samples = 200;
        p2.k_prime_factor = 2.0f;  // realistic default
        p2.gamma_enq = 0.15f;
        p2.gamma_term = 0.35f;
        auto aq_default = ArcFlare::ArcFlareIndex::fromNGT(idx_path, p2);
    ...
    aq.save(save_path);
    auto aq2 = ArcFlare::ArcFlareIndex::load(save_path);
...
// ---------------------------------------------------------------------------
int main() {
    std::cout << "=== ArcFlareIndex Integration Tests ===\n";
    try {
