// tests/arcflare/test_bq_distance.cpp
// TDD tests for ArcFlare::bqDistance (interleaved 2-pointer API)
...
static void testIdenticalVectors() {
  ...
  float d = ArcFlare::bqDistance(p, q, words, D);
...
static void testSymmetry() {
  ...
  float dpq = ArcFlare::bqDistance(p, q, words, D);
  float dqp = ArcFlare::bqDistance(q, p, words, D);
...
static void testKnownValue() {
  ...
  d = ArcFlare::bqDistance(p, q, words, D);
  EXPECT_NEAR(d, 1.0f, 1e-9f);
...
static void testNonMultipleOf8Words() {
    ...
    float got = ArcFlare::bqDistance(p.data(), q.data(), words, D);
