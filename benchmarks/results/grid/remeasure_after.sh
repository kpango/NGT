...
# searchV2 visit-cap/bucketing/bounded-rerank optimization. NGTAQ swept over
...
ng_sweep () { # $1=ds $2=ge $3=np   -> writes ngtaq_after_<ds>_t1.log
  local ds="$1" ge="$2" np="$3" hdf5="$DATA/$1.hdf5" idx="$IDX/ngtaq_$1"
  local log="$LOGS/ngtaq_after_${ds}_t1.log"; : > "$log"
  ...
  echo "[$ds] NGTAQ after: $(grep -c agg_QPS "$log") pts"
}
...
for spec in \
  ...
  python3 "$LOGS/compare_grid.py" "$LOGS/ngtaq_after_${ds}_t1.log" "$LOGS/qbg_after_${ds}_t1.log" "$qlog" "$ds AFTER (1T)" | tee "$LOGS/grid_after_${ds}.txt"
done
