...
# Build QBG + QG (if missing), wait for the ArcFlare fp16 index, then bench all three
...
# 3) wait for the ArcFlare fp16 index (may still be building in another job)
while [ ! -s "$IDX/arcflare_$ds/aqindex" ]; do sleep 30; done
sleep 5
echo "[$ds] ArcFlare index ready $(date)"
...
for gt in 0.02 0.05 0.10 0.15 0.20 0.30 0.40 0.55 0.70 0.90; do
  ./ann_bench "$IDX/arcflare_$ds" "$hdf5" 10 "$ge" "$gt" 3 1 0 2>/dev/null >> "$NLOG"
done
./ann_bench "$IDX/arcflare_$ds" "$hdf5" 10 "$ge" 0.70 10 1 20 2>/dev/null >> "$NLOG"
./ann_bench "$IDX/arcflare_$ds" "$hdf5" 10 "$ge" 0.90 20 1 40 2>/dev/null >> "$NLOG"
echo "[$ds] ArcFlare benched ($(grep -c agg_QPS "$NLOG") pts) $(date)"
