...
# qbg_bench, and qg_bench. A cell PASSES only if ArcFlare >= 2x the stronger competitor.
import sys, re
...
def main():
    if len(sys.argv) < 4:
        print("usage: compare_grid.py <arcflare_log> <qbg_log> <qg_log> [label]"); sys.exit(1)
    ...
    print(f"# {label}  ArcFlare_maxrecall={ng_max:.4f}  "
          f"(pts: arcflare={len(ng)} qbg={len(qb)} qg={len(qg)})")
