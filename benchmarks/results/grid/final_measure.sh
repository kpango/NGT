#!/usr/bin/env bash
# final_measure.sh — definitive after-all-optimizations grid (dec7f18 binary, build_arcflare2).
# ArcFlare swept with the CORRECT knobs: L2 -> visit-cap (max_visits) as the low-recall lever;
...
WTB=/home/kpango/go/src/github.com/kpango/NGT/.claude/worktrees/arcflare-beat-qbg/build_arcflare2
...
l2_arcflare () { # $1=ds  -> visit-cap frontier
  local ds="$1" hdf5="$DATA/$1.hdf5" idx="$IDX/arcflare_$1" log="$LOGS/arcflare_final_${1}_t1.log"; : > "$log"
  for gt in 0.10 0.40 0.90; do
    for mv in 15 30 60 150 400 1000 0; do
      ./ann_bench "$idx" "$hdf5" 10 0.20 "$gt" 3 1 0 "$mv" 2>/dev/null >> "$log"
    done
  done
  echo "[$ds] ArcFlare-final $(grep -c agg_QPS "$log") pts"
}
ang_arcflare () { # $1=ds  -> seeds_per_cluster frontier
  local ds="$1" hdf5="$DATA/$1.hdf5" idx="$IDX/arcflare_$1" log="$LOGS/arcflare_final_${1}_t1.log"; : > "$log"
...
comp () { # $1=ds $2=metric
  ...
  python3 "$LOGS/compare_grid.py" "$LOGS/arcflare_final_${1}_t1.log" "$LOGS/qbg_final_${1}_t1.log" "$qlog" "$ds FINAL (1T)" | tee "$LOGS/grid_final_${1}.txt"
}

echo "=== final_measure start $(date) ==="
l2_arcflare sift-128-euclidean;  comp sift-128-euclidean l2
ang_arcflare glove-100-angular;  comp glove-100-angular angular
l2_arcflare gist-960-euclidean;  comp gist-960-euclidean l2
