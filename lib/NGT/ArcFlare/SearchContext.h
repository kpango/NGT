...
#include <cstddef>
#include <utility>
#include "NGT/ArcFlare/ADCTable.h"    // NGT::ArcFlare::ADCQueryState
#include "NGT/ArcFlare/GlobalPQ4.h"   // NGT::ArcFlare::GlobalPQ4LUT

namespace ArcFlare {

// === hoisted from ArcFlareIndex.cpp in Step 0 (verbatim definitions) ===
...
// hoisted from ArcFlareIndex.cpp searchV2 neighbor-expansion (was a function-local struct)
struct NbrCand { uint32_t cid; uint32_t id; };

// hoisted from ArcFlareIndex.cpp searchV2 seeding (was a function-local struct)
struct SeedScore { float score; uint32_t id; };

// ADC slot-cache metadata. Defined locally in ArcFlareIndex.cpp's searchV2 today; mirrored
...
// NOTE: ADCQueryState/GlobalPQ4LUT live in nested namespace NGT::ArcFlare (legacy); the hoisted types and SearchContext/Pool live in top-level ArcFlare — hence the NGT::ArcFlare:: qualification on those two members.
struct SearchContext {
    ...
    NGT::ArcFlare::ADCQueryState adc;      // adc_tl
    ...
    NGT::ArcFlare::GlobalPQ4LUT batch_lut; // batch_lut_tl
