...
public:
    ...
    uint32_t nearest_fp16(const float* x) const {
        ...
        for (uint32_t k = 0; k < K_; ++k) {
            ...
            float d = NGT::ArcFlare::l2_sq_f32_fp16(x, cp + (size_t)k * D_, D_);
    ...
    uint32_t nearest_2level(const float* x, int probe = 4) const {
        ...
        for (uint32_t s = 0; s < S_; ++s) {
            float d = NGT::ArcFlare::l2_sq(x, super_centroids_.data() + (size_t)s * D_, D_);
            if (d < bestd[P - 1]) {
                int j = P - 1;
        ...
        for (int i = 0; i < P; ++i) {
            ...
            for (uint32_t m = lo; m < hi; ++m) {
                ...
                float d = NGT::ArcFlare::l2_sq(x, centroids_.data() + (size_t)c * D_, D_);
...
private:
    ...
    void build_fp16_cache() const {
        ...
        for (size_t i = 0; i < tmp.size(); ++i)
            tmp[i] = NGT::ArcFlare::float_to_fp16(centroids_[i]);
    ...
    void build_2level() const {
        ...
        for (int iter = 0; iter < 8; ++iter) {
            ...
            for (uint32_t c = 0; c < K_; ++c) {
                ...
                for (uint32_t s = 0; s < S; ++s) {
                    float d = NGT::ArcFlare::l2_sq(cv, sc.data() + (size_t)s * D_, D_);
                    if (d < bd) { bd = d; bs = s; }
                }
