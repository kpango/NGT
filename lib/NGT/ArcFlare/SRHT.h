...
class SRHT {
public:
    // Load constructor: reconstruct from a pre-built diagonal (e.g., loaded from disk).
    explicit SRHT(std::vector<float> diag)
        : D_((int)diag.size()), skip_diagonal_(false), diag_(std::move(diag))
    {
        assert((D_ & (D_ - 1)) == 0 && D_ >= 2);
