...
public:
    ...
    void resetEdges(const std::vector<std::vector<uint32_t>>& adj) {
        ...
        if (total > edge_ids_.max_size()) {
            fprintf(stderr, "[ArcFlare][FATAL] resetEdges overflow: total_edges=%zu N=%zu max=%zu\n",
    ...
    // v2_records_flat_ consistently. Used by ArcFlareIndex::reorderForLocality (BFS relayout).
    // o2n[i] = new id of old node i (must be a bijection over [0,N)). Caller remaps the
    // ArcFlareIndex-side node-keyed arrays (gpq4/sq8/raw_flat/etc.) and rebuilds the gpq4 store.
    ...
    struct alignas(64) PackedV2Node {
        NGT::ArcFlare::VectorRecord rec;            // 38 bytes [0..37]
    ...
    void reserveV2(size_t n, int tier1_n = 16, int tier2_n = 16) {
        ...
        if (n > v2_records_flat_.max_size() / (size_t)v2_rec_stride_) {
            fprintf(stderr, "[ArcFlare][FATAL] reserveV2 overflow: n=%zu stride=%d bytes=%zu max=%zu\n",
    ...
    void setRecord(uint32_t node_id, const NGT::ArcFlare::VectorRecord& rec) {
    ...
    void setRecordView(uint32_t node_id, NGT::ArcFlare::VectorRecordView view) {
        memcpy(v2_records_flat_.data() + (size_t)node_id * v2_rec_stride_, view.ptr, v2_rec_stride_);
    }

    NGT::ArcFlare::VectorRecordView getRecordView(uint32_t node_id) {
        return NGT::ArcFlare::vrec_view(v2_records_flat_.data(), node_id, v2_tier1_n_, v2_tier2_n_);
    }

    // D=128 only: reinterpret flat bytes as VectorRecord
    const NGT::ArcFlare::VectorRecord& getRecord(uint32_t node_id) const {
        return *reinterpret_cast<const NGT::ArcFlare::VectorRecord*>(
            v2_records_flat_.data() + (size_t)node_id * 38);
    }

    NGT::ArcFlare::VectorRecordConstView getRecordConstView(uint32_t node_id) const {
        return NGT::ArcFlare::vrec_const_view(v2_records_flat_.data(), node_id, v2_tier1_n_, v2_tier2_n_);
    }
    ...
    float gpq4BlockNorm(const uint8_t* block, int n) const {
        ...
        return NGT::ArcFlare::bf16_to_float(normp[n]);
    }
...
inline void SoAGraph::buildGPQ4(int M, const uint8_t* node_codes, const float* node_norm_sq) {
    ...
    if (blk_bytes && total_blocks > gpq4_blocks_.max_size() / blk_bytes) {
        fprintf(stderr, "[ArcFlare][FATAL] buildGPQ4 overflow: total_blocks=%zu blk_bytes=%zu N=%zu M=%d\n",
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        for (uint32_t blk = 0; blk * BLK < nn; ++blk) {
            ...
            for (int n = 0; n < n_real; ++n) {
                ...
                normp[n] = NGT::ArcFlare::float_to_bf16(node_norm_sq[nbr]);
            }
        }
    }
}
