// lib/NGT/ArcFlare/ArcFlareIndex.cpp
#include "NGT/ArcFlare/ArcFlareIndex.h"
#include "NGT/ArcFlare/DimUtils.h"
...
// AQLinearPool (pyglass-style ef-bounded sorted-array candidate frontier) hoisted
// to NGT/ArcFlare/SearchContext.h (P0 PRISM Task 2) so SearchContext can hold one.

// ---------------------------------------------------------------------------
// Private constructor
// ---------------------------------------------------------------------------
ArcFlareIndex::ArcFlareIndex(Property prop, BinaryQuantizer bq,
    ...
    , raw_flat_(std::move(raw_flat))
{
    ...
    // Resolve the six structural AQ_* flags once (single ctor path for fromNGT()+load()).
    // aq_cfg() reuses the existing env parse; this ctor body runs on every construction
    // path, guaranteeing rt_cfg_ is populated regardless of how the index was created.
    rt_cfg_.use_global_routing = aq_cfg().use_global_routing;
    rt_cfg_.batch_routing      = aq_cfg().batch_routing;
    rt_cfg_.dist_lut           = aq_cfg().dist_lut;
    rt_cfg_.use_sq8            = aq_cfg().use_sq8;
    rt_cfg_.graph_entry        = aq_cfg().graph_entry;
    rt_cfg_.versioned_vis      = aq_cfg().versioned_vis;
}

// ---------------------------------------------------------------------------
// fromNGT
// ---------------------------------------------------------------------------
ArcFlareIndex ArcFlareIndex::fromNGT(const std::string& ngt_path, const Property& prop) {
    if (prop.dimension % 64 != 0)
        throw std::invalid_argument("ArcFlareIndex: dimension must be a multiple of 64");
    ...
    for (size_t i = 0; i < raw_flat.size(); ++i)
        raw_flat_h[i] = NGT::ArcFlare::float_to_fp16(raw_flat[i]);

    return ArcFlareIndex(prop, std::move(bq), std::move(graph),
...
// search
// ---------------------------------------------------------------------------
std::vector<SearchResult> ArcFlareIndex::search(
    const std::vector<float>& query, int k) const
{
    if (static_cast<int>(query.size()) < prop_.dimension)
        throw std::invalid_argument("ArcFlareIndex::search: query dimension mismatch");
    ...
    for (uint32_t id : cand_ids) {
        ...
        if (!is_cosine) {
            exact_dist = std::sqrt(NGT::ArcFlare::l2_sq_f32_fp16(query.data(), vec, D));
        } else {
            float dot = 0.0f;
            for (int j = 0; j < D; ++j) dot += qn[j] * NGT::ArcFlare::fp16_to_float(vec[j]);
            exact_dist = 1.0f - dot;
        }
...
// searchBatch
// ---------------------------------------------------------------------------
std::vector<std::vector<SearchResult>> ArcFlareIndex::searchBatch(
...
// insert
// ---------------------------------------------------------------------------
uint32_t ArcFlareIndex::insert(const std::vector<float>& vec) {
    if (static_cast<int>(vec.size()) < prop_.dimension)
        throw std::invalid_argument("ArcFlareIndex::insert: vector dimension mismatch");
    const int D = prop_.dimension;
    const int words = D / 64;
    ...
    for (int j = 0; j < D; ++j)
        raw_flat_.push_back(NGT::ArcFlare::float_to_fp16(vec[j]));
    ...
    if (prop_.metric == NGT::ObjectSpace::DistanceTypeAngle ||
        ...
        float norm_sq = 0.0f;
        for (int j = 0; j < D; ++j) {
            float f = NGT::ArcFlare::fp16_to_float(h[j]);
            norm_sq += f * f;
        }
        if (norm_sq > 0.0f) {
            float inv_norm = 1.0f / std::sqrt(norm_sq);
            for (int j = 0; j < D; ++j)
                h[j] = NGT::ArcFlare::float_to_fp16(NGT::ArcFlare::fp16_to_float(h[j]) * inv_norm);
        }
    }
...
// remove
// ---------------------------------------------------------------------------
void ArcFlareIndex::remove(uint32_t id) {
    std::unique_lock<std::shared_mutex> lock(graph_->mutex());
...
// size
// ---------------------------------------------------------------------------
size_t ArcFlareIndex::size() const {
...
static_assert(sizeof(std::streamsize) >= 8, "ArcFlare index I/O assumes 64-bit streamsize");

void ArcFlareIndex::save(const std::string& path) const {
    std::ofstream os(path, std::ios::binary);
    if (!os) throw std::runtime_error("ArcFlareIndex::save: cannot open " + path);
    ...
    os.flush();
    if (!os) throw std::runtime_error("ArcFlareIndex::save: write error on " + path);
}

// ---------------------------------------------------------------------------
// load
// ---------------------------------------------------------------------------
ArcFlareIndex ArcFlareIndex::load(const std::string& path) {
    std::ifstream is(path, std::ios::binary);
    if (!is) throw std::runtime_error("ArcFlareIndex::load: cannot open " + path);
    ...
    if (!is) throw std::runtime_error("ArcFlareIndex::load: failed to read header from " + path);
    ...
    // fp16 versioned format: [magic(4)][prop_size(4)][prop_bytes(prop_size)]
    {
        ...
        if (!is) throw std::runtime_error("ArcFlareIndex::load: failed to read prop_size from " + path);
        ...
        if (!is) throw std::runtime_error("ArcFlareIndex::load: failed to read property from " + path);
    ...
    if (prop.dimension <= 0 || prop.dimension > 65536)
        throw std::runtime_error("ArcFlareIndex::load: invalid dimension in file");
    ...
    if (!is) throw std::runtime_error("ArcFlareIndex::load: failed to read entry point count");
    ...
    if (n_ep > static_cast<uint64_t>(graph->size()))
        throw std::runtime_error("ArcFlareIndex::load: n_ep exceeds graph size (file corrupt?)");
    ...
    if (!is) throw std::runtime_error("ArcFlareIndex::load: failed to read vec count");
    ...
    if (n_elems > max_reasonable)
        throw std::runtime_error("ArcFlareIndex::load: n_elems exceeds limit (file corrupt?)");
    ...
    if (!is)
        throw std::runtime_error("ArcFlareIndex::load: stream error reading " + path);
...
// selectEntryPoints
// ---------------------------------------------------------------------------
std::vector<uint32_t> ArcFlareIndex::selectEntryPoints(
...
// ---------------------------------------------------------------------------
ArcFlareIndex ArcFlareIndex::fromNGTv2(const std::string& ngt_path, const Property& prop) {
    const int D_orig = prop.dimension;
    const int D = NGT::ArcFlare::pad_dim_for_v2(D_orig);  // pad to next power-of-2 divisible by 64
    ...
    fprintf(stderr, "[ArcFlare] Loaded %zu vectors D_orig=%d D_eff=%d\n", N, D_orig, D);
    ...
    if (prop.metric == NGT::ObjectSpace::DistanceTypeAngle ||
        ...
        fprintf(stderr, "[ArcFlare] Angular: L2-normalized %zu vectors (%d degenerate holes)\n",
                N, n_zero);
    }

    // ---- 2. SRHT: rotate all vectors ----
    const uint64_t seed = 0xCAFEBABE12345678ULL;
    auto srht = std::make_unique<NGT::ArcFlare::SRHT>(D, seed);
    ...
    uint32_t K = (prop.k_clusters > 0)
                 ...
                 : NGT::ArcFlare::select_k(N);
    auto kmeans = std::make_unique<NGT::ArcFlare::KMeansCentering>(K, D, seed ^ 0xFFFF);
    fprintf(stderr, "[ArcFlare] K-means K=%u...\n", K);
    ...
    auto pca = std::make_unique<NGT::ArcFlare::PCAProjector>(D, 32, seed ^ 0x1234);
    size_t fit_n = std::min(N, (size_t)262144);
    fprintf(stderr, "[ArcFlare] PCA fit on %zu residuals...\n", fit_n);
    ...
    fprintf(stderr, "[ArcFlare] Training %d PQ sub-codebooks (K=%d, D_sub=%d) on SRHT residuals...\n",
            M_PQ, K_PQ, D_sub);
    for (int sub = 0; sub < M_PQ; ++sub) {
        ...
        NGT::ArcFlare::KMeansCentering sub_km(K_PQ, D_sub, seed ^ (0xABCD1234ULL + (uint64_t)sub));
    ...
    fprintf(stderr, "[ArcFlare] PQ sub-codebooks done.\n");
    ...
    {
        fprintf(stderr, "[ArcFlare] Training %d GLOBAL PQ sub-codebooks on rotated vectors...\n", M_PQ);
        for (int sub = 0; sub < M_PQ; ++sub) {
            std::vector<float> sub_data(N * (size_t)D_sub);
            for (size_t i = 0; i < N; ++i)
                memcpy(sub_data.data() + i*D_sub,
                       ...
                       (size_t)D_sub * sizeof(float));
            NGT::ArcFlare::KMeansCentering sub_km(K_PQ, D_sub, seed ^ (0x5EED9001ULL + (uint64_t)sub));
            sub_km.train(sub_data.data(), N, 262144, 50);
            for (int code = 0; code < K_PQ; ++code)
        ...
        fprintf(stderr, "[ArcFlare] GLOBAL PQ done.\n");
    }
    ...
    std::vector<float> gpq4_cb((size_t)GM * NGT::ArcFlare::GPQ4_K * GDsub, 0.f);
    std::vector<uint8_t> gpq4_codes(N * (size_t)GM, 0);
    std::vector<float>   gpq4_norm_sq(N, 0.f);
    {
        fprintf(stderr, "[ArcFlare] Training %d GLOBAL PQ-16 sub-codebooks (K=16, D_sub=%d) on rotated vectors...\n",
                GM, GDsub);
        for (int sub = 0; sub < GM; ++sub) {
            ...
            NGT::ArcFlare::KMeansCentering sub_km(NGT::ArcFlare::GPQ4_K, GDsub,
                                               seed ^ (0x4B16C0DEULL + (uint64_t)sub));
            sub_km.train(sub_data.data(), N, 262144, 50);
            for (int code = 0; code < NGT::ArcFlare::GPQ4_K; ++code)
                memcpy(gpq4_cb.data() + (sub*NGT::ArcFlare::GPQ4_K + code)*GDsub,
                       sub_km.centroid(code),
                       (size_t)GDsub * sizeof(float));
        }
        ...
        fprintf(stderr, "[ArcFlare] GPQ4 encoding with eta=%.2f (%s)\n", gpq4_eta,
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            float rns = 0.f;
            if (gpq4_eta > 1.0f) {
                NGT::ArcFlare::gpq4_encode_anisotropic(xr, gpq4_cb.data(), GM, GDsub, gpq4_eta, codes, rns);
            } else {
                for (int sub = 0; sub < GM; ++sub)
                    codes[sub] = NGT::ArcFlare::gpq4_encode_sub(
                        xr + sub*GDsub, gpq4_cb.data() + (size_t)sub*NGT::ArcFlare::GPQ4_K*GDsub, GDsub, rns);
            }
            gpq4_norm_sq[i] = rns;
        }
        fprintf(stderr, "[ArcFlare] GLOBAL PQ-16 done.\n");
    }
    ...
    if (build_sq8) {
        ...
        if (sq8_align <= 0 || N > sq8_codes.max_size() / (size_t)sq8_align) {
            fprintf(stderr, "[ArcFlare][FATAL] SQ8 alloc overflow: N=%zu sq8_align=%d bytes=%zu max=%zu\n",
        ...
        fprintf(stderr, "[ArcFlare] Encoding symmetric SQ8 codes (dim_align=%d)...\n", sq8_align);
        #pragma omp parallel for schedule(static)
        for (size_t i = 0; i < N; ++i) {
            if (is_hole[i]) continue;
            const float* xr = rotated.data() + i * D;
            int8_t* code = sq8_codes.data() + i * (size_t)sq8_align;
            sq8_max[i] = NGT::ArcFlare::sq8_encode_sym(xr, code, D);  // [D, sq8_align) stays 0
            float ns = 0.f;
            for (int d = 0; d < D; ++d) ns += xr[d] * xr[d];
            sq8_norm[i] = ns;
        }
        fprintf(stderr, "[ArcFlare] SQ8 codes done.\n");
    } else {
        fprintf(stderr, "[ArcFlare] SQ8 build-encoding skipped (AQ_BUILD_SQ8 unset; opt-in).\n");
    }
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        view.set_norm_fp16(NGT::ArcFlare::float_to_fp16(std::sqrt(norm2)));
    ...
    // Construct index
    ArcFlareIndex idx(prop, std::move(bq), std::move(graph),
                   std::move(entry_points), std::move(raw_flat_h));
    idx.is_v2_ = true;
    ...
    // Build transposed codebook [M][D_sub][K] for AVX2 FMA LUT build
    {
        ...
        NGT::ArcFlare::build_tier2_codebook_T(
    ...
    {
        idx.gpq4_codebook_T_.resize((size_t)GM * GDsub * NGT::ArcFlare::GPQ4_K);
        NGT::ArcFlare::build_tier2_codebook_T(
            idx.gpq4_codebook_.data(), GM, NGT::ArcFlare::GPQ4_K, GDsub,
    ...
    idx.has_gpq4_ = true;
    fprintf(stderr, "[ArcFlare] GPQ4 neighbor-code store built (M=%d, D_sub=%d).\n", GM, GDsub);
    ...
    // AQ_REORDER=bfs|rcm selects an alternate mode. Requires a tombstone-free fresh build (true).
    {
        ...
        if (!(skip && std::atoi(skip) != 0)) {
            ...
            fprintf(stderr, "[ArcFlare] Reordering for cache locality (mode=%s)...\n",
                    rm == ReorderMode::GORDER ? "gorder" : rm == ReorderMode::RCM ? "rcm" : "bfs");
            idx.reorderForLocality(rm);
        }
    }

    fprintf(stderr, "[ArcFlare] Build complete. N=%zu K=%u\n", N, K);
    return idx;
}
...
// ---------------------------------------------------------------------------
float ArcFlareIndex::buildGlobalLUT(const std::vector<float>& query, float* lut,
    ...
    if (is_angular_) {
        ctx.q_norm_lut.assign(query.begin(), query.begin() + std::min(D_orig, D));
        float n2 = 0.f;
        for (float x : ctx.q_norm_lut) n2 += x * x;
        if (n2 > 1e-12f) { float inv = 1.f/std::sqrt(n2); for (float& x : ctx.q_norm_lut) x *= inv; }
        q_src = ctx.q_norm_lut.data();
    }

    // Zero-pad to D, then SRHT-rotate (codebook was trained on rotated vectors directly).
    ctx.q_padded_lut.assign(static_cast<size_t>(D), 0.f);
    std::copy(q_src, q_src + std::min(D_orig, D), ctx.q_padded_lut.begin());
    ctx.q_rot_lut.resize(static_cast<size_t>(D));
    srht_v2_->apply(ctx.q_padded_lut.data(), ctx.q_rot_lut.data());

    // LUT[sub][code] = <q_rot_sub, global_centroid_sub>. Reuse the tier-2 fast builder.
    NGT::ArcFlare::build_tier2_lut_fast_m(ctx.q_rot_lut.data(), M_PQ,
                                        global_pq_codebook_T_.data(), lut);

    float q_norm_sq = 0.f;
    for (int d = 0; d < D; ++d) q_norm_sq += ctx.q_rot_lut[d] * ctx.q_rot_lut[d];
    return q_norm_sq;
}

float ArcFlareIndex::globalPQDist(uint32_t node_id, const float* lut, float q_norm_sq) const {
    const int M_PQ = (m_pq_ > 0) ? m_pq_ : 16;
    const uint8_t* codes = global_codes_.data() + (size_t)node_id * M_PQ;
    // <q_rot, x_rot_pq> via the global LUT (one LUT scores any node).
    float ip = NGT::ArcFlare::tier2_adc_pq_m(lut, codes, M_PQ);
...
// ---------------------------------------------------------------------------
float ArcFlareIndex::buildGlobalLUT16(const std::vector<float>& query,
    ...
    ctx.gpq4_ip_scratch.resize((size_t)M_PQ * NGT::ArcFlare::GPQ4_K);
    if (dist_lut) {
        // QG-style squared-distance table: kernel accumulates L2 directly.
        NGT::ArcFlare::gpq4_dist_table(ctx.q_rot_lut.data(), M_PQ, gpq4_codebook_T_.data(), D_sub, ctx.gpq4_ip_scratch.data());
    } else {
        NGT::ArcFlare::gpq4_ip_table(ctx.q_rot_lut.data(), M_PQ, gpq4_codebook_T_.data(), D_sub, ctx.gpq4_ip_scratch.data());
    }
    NGT::ArcFlare::gpq4_build_lut(ctx.gpq4_ip_scratch.data(), M_PQ, lut);
    if (ip_out) std::copy(ctx.gpq4_ip_scratch.begin(), ctx.gpq4_ip_scratch.end(), ip_out);
...
std::pair<double,double> ArcFlareIndex::graphLocalityStats() const {
    const size_t N = graph_->size();
...
void ArcFlareIndex::reorderForLocality(ReorderMode mode) {
    const size_t N = graph_->size();
    if (N == 0) return;
    ...
    // 3. Permute ArcFlareIndex node-keyed arrays. Helper for a row-major [N*stride] vector.
...
void ArcFlareIndex::rotateForDiag(const float* q, int q_dim, float* out) const {
    const int D = (d_eff_ > 0) ? d_eff_ : prop_.dimension;
...
// ---------------------------------------------------------------------------
// Build SearchParameters from aq_cfg()/members (reproducing today's exact behavior for
// searchV2(query, k, ...)). Shared by the legacy delegator below and rebuildGraphSelf.
SearchParameters ArcFlareIndex::buildLegacyParams(int k, float gamma_enq, float gamma_term,
    ...
    p.k_prime_factor    = searcher_.k_prime_factor;
    p.n_cluster_seeds   = prop_.n_cluster_seeds;
    p.seeds_per_cluster = prop_.seeds_per_cluster;
    p.n_probe           = 0;  // 0 = worker derives default (is_angular ? 20 : 3)
    return p;
}

// Legacy signature: build SearchParameters and delegate to the injected overload.
std::vector<SearchResult> ArcFlareIndex::searchV2(const std::vector<float>& query, int k,
        float gamma_enq, float gamma_term, int rerank_factor, int max_visits) const {
    SearchParameters p = buildLegacyParams(k, gamma_enq, gamma_term, rerank_factor, max_visits);
    SearchContextGuard g(*ctx_pool_);
    return searchV2(query, p, *g);
}

std::vector<SearchResult> ArcFlareIndex::searchV2(const std::vector<float>& query,
        const SearchParameters& params, SearchContext& ctx) const
{
    int   k             = params.k;
    float gamma_enq     = params.gamma_enq;
    float gamma_term    = params.gamma_term;
    int   rerank_factor = params.rerank_factor;
    int   max_visits    = params.max_visits;
    ...
    // Routing selectors (single source of truth: rt_cfg_, snapshot of aq_cfg() at ctor). batch_routing default ON =
    ...
    const bool use_batch_e  = hasGPQ4() && rt_cfg_.batch_routing;
    const bool use_global_e = (has_global_pq_ && rt_cfg_.use_global_routing) && !use_batch_e;
    const bool use_coarse_e = use_batch_e || use_global_e;

    // 0. Angular/Cosine: L2-normalize query (over D_orig dims)
    const float* q_src = query.data();
    if (is_angular_) {
        ctx.q_normalized.assign(query.begin(), query.begin() + std::min(D_orig, D));
        float norm2 = 0.f;
        for (float x : ctx.q_normalized) norm2 += x * x;
        if (norm2 > 1e-12f) {
            float inv = 1.f / std::sqrt(norm2);
            for (float& x : ctx.q_normalized) x *= inv;
        }
        q_src = ctx.q_normalized.data();
    }

    // Query padding: zero-pad to D (for D_orig < D, e.g., GloVe-100 → D=128)
    // Pooled scratch buffers (SearchContext): reused across queries to avoid
    // per-query heap allocation overhead (~50KB for D=1024, significant for QPS).
    // ADC state: q_int8 buffer lives in the pooled ctx.adc struct (one alloc per ctx).
    // Using a reference alias so all existing `adc.*` accesses compile unchanged.
    ctx.q_padded.assign(static_cast<size_t>(D), 0.f);
    std::copy(q_src, q_src + std::min(D_orig, D), ctx.q_padded.begin());
    const float* q_ptr = ctx.q_padded.data();

    // 1. Rotate query
    ctx.q_rot.resize(static_cast<size_t>(D));
    AQ_MARK();  // [AQ_PROFILE] start SRHT sub-timer (within setup)
    srht_v2_->apply(q_ptr, ctx.q_rot.data());
    ...
    const int cq_probe = params.cq_probe;
    uint32_t active_cid = use_coarse_e ? kmeans_v2_->nearest_2level(ctx.q_rot.data(), cq_probe)
                                       : kmeans_v2_->nearest_public(ctx.q_rot.data());
    ctx.q_res.resize(static_cast<size_t>(D));
    ...
    if (static_cast<int>(ctx.adc.q_int8.size()) < D)
        ctx.adc.q_int8.assign(static_cast<size_t>(D), int8_t(0));
    NGT::ArcFlare::ADCQueryState& adc = ctx.adc;
    ...
    if (!use_coarse_e) {
        NGT::ArcFlare::compute_residual_and_tier1(
            ctx.q_rot.data(), kmeans_v2_->centroid(active_cid), D,
            ctx.q_res.data(), adc.q_norm_sq, adc.q_int8.data(), adc.q_sum);
    ...
    if (!use_coarse_e)
        ctx.q_res_init.assign(ctx.q_res.begin(), ctx.q_res.end());
    ...
    // Cache-sizing reads the per-query n_probe (params.n_probe) so the worker stays stateless;
    // captured by value so the lambda holds no reference to the index.
    const int n_probe_param = params.n_probe;
    std::call_once(*cluster_members_once_, [this, N, n_probe_param]() {
        ...
        // n_probe (params.n_probe) may be set before the first search triggers this call_once.
        const int default_nbrs = is_angular_ ? 20 : 4;
        const int CLUSTER_NBRS = std::max(default_nbrs, n_probe_param);
        ...
        for (uint32_t c = 0; c < K; ++c) {
            ...
            for (uint32_t c2 = 0; c2 < K; ++c2) {
                ...
                float d2 = NGT::ArcFlare::KMeansCentering::l2sq(cc, cc2, Dim);
    ...
    // BATCH path. Pooled so its backing array persists across queries (no malloc).
    AQLinearPool& lp = ctx.lp;
    // Pooled results buffer: capacity persists across queries (no malloc on steady-state).
    ctx.results.clear();
    if (ctx.results.capacity() < static_cast<size_t>(k_beam * 30))
        ctx.results.reserve(static_cast<size_t>(k_beam * 30));
    ...
    const bool use_versioned_vis = rt_cfg_.versioned_vis;
    if (use_versioned_vis) {
        if (ctx.vis_ver.size() < N) ctx.vis_ver.assign(N, 0);
        if (++ctx.vis_cur == 0) {                            // wrap: clear stamps, restart at 1
            std::fill(ctx.vis_ver.begin(), ctx.vis_ver.end(), uint16_t(0));
            ctx.vis_cur = 1;
        }
    } else {
        ctx.vis.assign((N + 63) / 64, 0ULL);
    }
    const uint16_t vis_cur = ctx.vis_cur;
    uint16_t* __restrict vis_ver = ctx.vis_ver.data();
    auto is_visited = [&](uint32_t id) -> bool {
        return use_versioned_vis ? (vis_ver[id] == vis_cur)
                                 : ((ctx.vis[id >> 6] >> (id & 63)) & 1ULL);
    };
    auto mark_visited = [&](uint32_t id) {
        if (use_versioned_vis) vis_ver[id] = vis_cur;
        else                   ctx.vis[id >> 6] |= 1ULL << (id & 63);
    };
    ...
    const int ef_env = params.ef;  // AQ_EF (0 = derive from max_visits); see aq_cfg()
    ...
    // Pooled flat int8 buffer (ADC_SLOTS × D) + scalar metadata avoids
    // per-query heap allocation (~8KB for D=1024). ADC_SLOTS / ADCSlotMeta live in
    // NGT/ArcFlare/SearchContext.h (namespace ArcFlare; resolved here by unqualified lookup).
    ...
    if (!use_coarse_e) {
        ctx.adc_int8.resize(static_cast<size_t>(ADC_SLOTS) * static_cast<size_t>(D));
        for (auto& m : ctx.adc_meta) m.cid = UINT32_MAX;
        // Slot 0 = initial cluster (ADC state already computed above)
        ctx.adc_meta[0] = {active_cid, adc.q_norm_sq, adc.q_norm, adc.q_sum};
        std::memcpy(ctx.adc_int8.data(), adc.q_int8.data(), static_cast<size_t>(D));
    }
    ...
    auto adc_dist = [&](const NGT::ArcFlare::VectorRecordConstView& rec) -> float {
        float norm_x = NGT::ArcFlare::fp16_to_float(rec.norm_fp16());
        float t1 = NGT::ArcFlare::tier1_adc_fast_d(adc.q_int8.data(), rec.tier1(), adc.q_sum, D);
        float t1_ip = adc.q_norm * norm_x * NGT::ArcFlare::RABITQ_SCALE * t1 * inv_sqrt_D / 127.f;
        return adc.q_norm_sq + norm_x * norm_x - 2.0f * t1_ip;
    };

    // Rebuild tier-1 ADC tables on cluster boundary crossing, with cache lookup.
    // Cache hit (~10ns): restore from the injected ctx's L1-resident flat slot buffer, skip recompute.
    ...
    auto maybe_rebuild_adc = [&](uint32_t cid) {
        ...
        for (int s = 0; s < ADC_SLOTS; ++s) {
            if (ctx.adc_meta[s].cid == cid) {
                active_cid    = cid;
                q_norm_sq     = ctx.adc_meta[s].q_norm_sq;
                adc.q_norm_sq = ctx.adc_meta[s].q_norm_sq;
                adc.q_norm    = ctx.adc_meta[s].q_norm;
                adc.q_sum     = ctx.adc_meta[s].q_sum;
                std::memcpy(adc.q_int8.data(),
                            ctx.adc_int8.data() + static_cast<size_t>(s) * static_cast<size_t>(D),
        ...
        ctx.adc_meta[slot] = {cid, adc.q_norm_sq, adc.q_norm, adc.q_sum};
        std::memcpy(ctx.adc_int8.data() + static_cast<size_t>(slot) * static_cast<size_t>(D),
    ...
    if (!use_coarse_e) {
        ctx.t2_lut.resize(static_cast<size_t>(M_PQ) * 256);
        NGT::ArcFlare::build_tier2_lut_fast_m(ctx.q_res_init.data(), M_PQ,
    ...
    // Batch / global-PQ routing selectors — same rt_cfg_ source as use_batch_e above
    // (single source of truth; the two sites are now guaranteed to agree). batch wins if both.
    const bool use_batch = hasGPQ4() && rt_cfg_.batch_routing;
    const bool use_global_pq = (has_global_pq_ && rt_cfg_.use_global_routing) && !use_batch;

    float q_ns_global = 0.f;
    if (use_global_pq) {
        ctx.global_lut.resize(static_cast<size_t>(M_PQ) * 256);
        q_ns_global = buildGlobalLUT(query, ctx.global_lut.data(), ctx);
    }
    ...
    const bool use_dist_lut = use_batch && rt_cfg_.dist_lut;
    if (use_batch) {
        ...
        ctx.batch_ip.resize((size_t)gpq4MPQ() * NGT::ArcFlare::GPQ4_K);
        AQ_MARK();  // [AQ_PROFILE] start dist-LUT build sub-timer (within setup)
        q_ns_batch = buildGlobalLUT16(query, ctx.batch_lut, ctx, ctx.batch_ip.data(), use_dist_lut);
    ...
    const bool use_sq8 = use_batch && rt_cfg_.use_sq8 && hasSQ8();
    ...
    const bool need_sq8_q = (use_sq8 || (params.rerank_sq8 > 0 && use_batch)) && hasSQ8();
    if (need_sq8_q) {
        ctx.q_sq8.assign((size_t)sq8_dim_align_, 0);
        q_max_sq8 = NGT::ArcFlare::sq8_encode_sym(ctx.q_rot.data(), ctx.q_sq8.data(), D);
        for (int d = 0; d < D; ++d) q_nsq_sq8 += ctx.q_rot[d] * ctx.q_rot[d];
    }
    auto sq8_dist1 = [&](uint32_t id) -> float {
        return sq8Dist(id, ctx.q_sq8.data(), q_max_sq8, q_nsq_sq8);
    };
    auto route_dist = [&](uint32_t id) -> float {
        return globalPQDist(id, ctx.global_lut.data(), q_ns_global);
    };
    ...
    auto batch_dist1 = [&](uint32_t id) -> float {
        ...
        if (use_dist_lut) return gpq4DistL2(id, ctx.batch_ip.data());  // distance-LUT: L2 directly
        return gpq4Dist(id, ctx.batch_ip.data(), q_ns_batch);
    };
    ...
    const int coarse_seeds_env = params.seeds;  // AQ_SEEDS (0 = default); see aq_cfg()
    const int N_CLUSTER_SEEDS =
        use_coarse ? (coarse_seeds_env > 0 ? coarse_seeds_env : 8)
                   : ((params.n_cluster_seeds > 0) ? params.n_cluster_seeds
    ...
    // ADC seeding genuinely needs the cross-cluster coverage). AQ_NPROBE / params.n_probe override.
    const bool batch_angular = is_angular_ && use_coarse;
    const int angular_default = batch_angular ? 1 : 20;
    const int n_probe = (params.n_probe > 0) ? params.n_probe
                                                 : (is_angular_ ? angular_default : 3);
    // SeedScore hoisted to NGT/ArcFlare/SearchContext.h (P0 PRISM Task 2).
    ...
    const bool graph_entry = rt_cfg_.graph_entry;  // AQ_GRAPH_ENTRY (default off); see aq_cfg()
    if (graph_entry && use_batch) {
        auto& scored = ctx.scored;
        scored.clear();
    ...
    }
    {
        ...
        // pooled buffers: the seed phase ran ~12-19us/query and these two vectors
        // were heap-allocated+freed EVERY query (probe_clusters ~n_probe, scored reserve
        // n_probe*200). Reuse persistent pooled storage instead.
        auto& probe_clusters = ctx.probe_clusters;
        ...
        if (is_angular_ && !use_coarse)
            ctx.t2_lut_probe.resize(static_cast<size_t>(M_PQ) * 256);
        for (uint32_t cid_p : probe_clusters) {
            ...
            if (is_angular_ && !use_coarse) {
                ...
                NGT::ArcFlare::build_tier2_lut_fast_m(ctx.q_res.data(), M_PQ,
                                                    tier2_codebook_T_.data(),
                                                    ctx.t2_lut_probe.data());
            }
            const float q_ns  = is_angular_ ? adc.q_norm_sq : q_norm_sq_initial;
            const float* lut_p = is_angular_ ? ctx.t2_lut_probe.data() : ctx.t2_lut.data();
            ...
            const bool seed_cap_topk = params.seed_cap_topk;  // AQ_SEED_CAP_TOPK; see aq_cfg()
            const int seed_cap = params.seeds_per_cluster;
            ...
            for (size_t mi = 0; mi < take; ++mi) {
                ...
                if (use_coarse) {
                    ...
                    float norm_x = NGT::ArcFlare::fp16_to_float(rec.norm_fp16());
                    float t2_ip  = NGT::ArcFlare::tier2_adc_pq_m(lut_p, rec.tier2(), M_PQ);
...
seeds_done:  // Phase 3 graph-only entry jumps here, skipping the IVF seeding block
    ...
    if (use_batch) {
        ...
        // Task 3: QG-style epsilon early-termination. ArcFlare's LinearPool otherwise pops the
        ...
        const float term_eps = params.term_eps;  // AQ_TERM_EPS (<0 = off, default); see aq_cfg()
        ...
        const float term_eps_fp16 = params.term_eps_fp16;  // AQ_TERM_EPS_FP16 (<0 = off); see aq_cfg()
        ...
        auto exact_fp16 = [&](uint32_t id) -> float {
            ...
            return NGT::ArcFlare::l2_sq_f32_fp16(q_ptr, raw_flat_.data() + (size_t)id * D, D);
        };
        while (lp.has_next()) {
            ...
            ctx.block_ip.resize((size_t)nblk * 16);
            for (uint32_t b = 0; b < nblk; ++b)
                NGT::ArcFlare::gpq4_batch_ip(blocks + (size_t)b * blk_bytes,
                                          ctx.batch_lut, ctx.block_ip.data() + (size_t)b * 16);
            if (use_dist_lut) {
                ...
                for (size_t ni = 0; ni < n_nbrs; ++ni) {
                    ...
                    lp.insert(u, ctx.block_ip[(size_t)b * 16 + pos]);
                }
            } else {
                ...
                for (size_t ni = 0; ni < n_nbrs; ++ni) {
                    ...
                    mark_visited(u);
                    const uint32_t b = (uint32_t)(ni / 16), pos = (uint32_t)(ni % 16);
                    float ip   = ctx.block_ip[(size_t)b * 16 + pos];
    ...
    while (!cand_q.empty()) {
        ...
        if (dk_tracker.size() >= static_cast<size_t>(k_beam) &&
            ...
            if (use_coarse) {
                ...
                float t2ip = NGT::ArcFlare::tier2_adc_pq_m(ctx.t2_lut.data(), rec_x.tier2(), M_PQ);
        ...
        if (use_batch) {
            // INVARIANT: unreachable when use_batch==true (outer use_batch block took `goto post_dabs`).
            // ctx.block_ip is shared with the LinearPool path; this guard documents why that is safe.
            assert(!use_batch && "ctx.block_ip aliasing: cand_q must not run with use_batch");
            const uint8_t* blocks = graph_->gpq4Blocks(x);
            const uint32_t nblk   = graph_->gpq4NumBlocks(x);
            if (blocks && nblk > 0) {
                graph_->prefetchGPQ4(x);
        ...
        // NbrCand hoisted to NGT/ArcFlare/SearchContext.h (P0 PRISM Task 2).
        auto& nbr_buf_tl = ctx.nbr_buf;
        nbr_buf_tl.clear();
...
post_dabs:  // Tech 2 batch LinearPool loop jumps here (skips the legacy cand_q loop)
    ...
    const int refine_mult_env = params.refine_mult;  // AQ_REFINE_MULT (0 = cascade floor); see aq_cfg()
    ...
    const int rerank_n_env = params.rerank_n;  // AQ_RERANK_N (>0 overrides refine_n); see aq_cfg()
    ...
    {
        const int sq8_rr = params.rerank_sq8;
        if (sq8_rr > 0 && use_batch && need_sq8_q && results.size() > (size_t)sq8_rr) {
            ...
            for (auto& [d, id] : results) d = sq8Dist(id, ctx.q_sq8.data(), q_max_sq8, q_nsq_sq8);
    ...
    if (use_batch && !results.empty()) {
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            ...
            ex.push_back({ NGT::ArcFlare::l2_sq_f32_fp16(q_ptr, v, D), (int)i });
        }
...
// ---------------------------------------------------------------------------
void ArcFlareIndex::rebuildGraphFromNGT(const std::string& ngt_path,
                                      float new_alpha,
                                      int   new_max_edges)
{
...
// ---------------------------------------------------------------------------
int ArcFlareIndex::fixHoleTombstones() {
    const int D = (d_eff_ > 0) ? d_eff_ : prop_.dimension;
    const size_t N = graph_->size();
    int n_fixed = 0;
    for (size_t i = 0; i < N; ++i) {
        ...
        float norm2 = 0.f;
        for (int d = 0; d < D; ++d) {
            float f = NGT::ArcFlare::fp16_to_float(h[d]);
            norm2 += f * f;
        }
        if (norm2 < 1e-12f) {
...
// ---------------------------------------------------------------------------
void ArcFlareIndex::rebuildGraphSelf(int   k_search,
                                   float gamma,
                                   int   n_threads,
                                   float new_alpha,
                                   int   new_max_edges)
{
    ...
    if (!raw_flat_.empty()) {
        ...
        for (int d = 0; d < D; ++d) dummy[d] = NGT::ArcFlare::fp16_to_float(h[d]);
    ...
    // across cluster boundaries. n_probe is injected per-search via SearchParameters
    // (rebuild_n_probe below); rebuild_max_seeds_ is set to match so all probed
    // centroids are used as seeds.
    int rebuild_n_probe = 0;  // 0 = worker derives default (legacy non-angular behavior)
    if (is_angular_) {
        const int K_clusters = static_cast<int>(kmeans_v2_ ? kmeans_v2_->num_clusters() : 0);
        if (K_clusters > 0) {
            rebuild_n_probe    = K_clusters;
            rebuild_max_seeds_ = K_clusters;
        }
    }

    // Base per-search params reproducing the legacy searchV2(q, k_search+1, gamma, gamma)
    // call, with n_probe injected for cross-cluster neighbor diversity during rebuild.
    SearchParameters rebuild_params =
        buildLegacyParams(k_search + 1, gamma, gamma, /*rerank_factor=*/0, /*max_visits=*/0);
    rebuild_params.n_probe = rebuild_n_probe;
...
#endif
    {
    // Per-thread search scratch: one SearchContext per worker thread, reused across all
    // of that thread's iterations (declared in the parallel region, never shared).
    SearchContext ctx;
#ifdef _OPENMP
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        const uint16_t* h = raw_flat_.data() + i * static_cast<size_t>(D);
        for (int d = 0; d < D; ++d) q[d] = NGT::ArcFlare::fp16_to_float(h[d]);
        // Search for k_search+1 (self may appear; skipped below). n_probe injected via params.
        // rebuild_params is const across threads (k already == k_search+1); ctx is per-thread.
        auto results = searchV2(q, rebuild_params, ctx);
    ...
    }  // end omp parallel region (per-thread SearchContext ctx)

    // Reset rebuild-only seed cap before any post-rebuild searches.
...
// saveV2 / loadV2
// ---------------------------------------------------------------------------
void ArcFlareIndex::saveV2(const std::string& dir) const {
    if (!is_v2_) return;
    // SoAGraph v2 records
...
void ArcFlareIndex::loadV2(const std::string& dir) {
    ...
    // SRHT
    {
        ...
        srht_v2_ = std::make_unique<NGT::ArcFlare::SRHT>((int)Dsz, 0);
    ...
    // K-means
    {
        ...
        kmeans_v2_ = std::make_unique<NGT::ArcFlare::KMeansCentering>(K, (int)Dim, 0);
    ...
    // PCA
    {
        ...
        pca_v2_ = std::make_unique<NGT::ArcFlare::PCAProjector>((int)Dim, (int)Top, 0);
    ...
    if (meta_version >= 3) {
        ...
        if (fc) {
            gpq4_codebook_.resize((size_t)M_cb * NGT::ArcFlare::GPQ4_K * D_sub);
            fc.read(reinterpret_cast<char*>(gpq4_codebook_.data()),
                    gpq4_codebook_.size() * sizeof(float));
            gpq4_codebook_T_.resize(gpq4_codebook_.size());
            NGT::ArcFlare::build_tier2_codebook_T(
                gpq4_codebook_.data(), M_cb, NGT::ArcFlare::GPQ4_K, D_sub,
