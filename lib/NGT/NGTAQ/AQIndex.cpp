...
#include "NGT/NGTAQ/SearchContext.h"
...
// to NGT/NGTAQ/SearchContext.h (P0 PRISM Task 2) so SearchContext can hold one.
...
// ---------------------------------------------------------------------------
float NGTAQIndex::buildGlobalLUT(const std::vector<float>& query, float* lut,
    ...
    NGT::NGTAQ::build_tier2_lut_fast_m(ctx.q_rot_lut.data(), M_PQ,
...
float NGTAQIndex::buildGlobalLUT16(const std::vector<float>& query,
    ...
    ctx.gpq4_ip_scratch.resize((size_t)M_PQ * NGT::NGTAQ::GPQ4_K);
    if (dist_lut) {
        // QG-style squared-distance table: kernel accumulates L2 directly.
        NGT::NGTAQ::gpq4_dist_table(ctx.q_rot_lut.data(), M_PQ, gpq4_codebook_T_.data(), D_sub, ctx.gpq4_ip_scratch.data());
    } else {
        NGT::NGTAQ::gpq4_ip_table(ctx.q_rot_lut.data(), M_PQ, gpq4_codebook_T_.data(), D_sub, ctx.gpq4_ip_scratch.data());
    }
    NGT::NGTAQ::gpq4_build_lut(ctx.gpq4_ip_scratch.data(), M_PQ, lut);
...
SearchParameters NGTAQIndex::buildLegacyParams(int k, float gamma_enq, float gamma_term,
...
std::vector<SearchResult> NGTAQIndex::searchV2(const std::vector<float>& query,
        const SearchParameters& params, SearchContext& ctx) const
{
    ...
    NGT::NGTAQ::ADCQueryState& adc = ctx.adc;
    ...
    // NGT/NGTAQ/SearchContext.h (namespace NGTAQ; resolved here by unqualified lookup).
    ...
    if (!use_coarse_e) {
        ...
        NGT::NGTAQ::build_tier2_lut_fast_m(ctx.q_res_init.data(), M_PQ,
    ...
    if (use_batch) {
        ...
        ctx.batch_ip.resize((size_t)gpq4MPQ() * NGT::NGTAQ::GPQ4_K);
    ...
    if (need_sq8_q) {
        ...
        q_max_sq8 = NGT::NGTAQ::sq8_encode_sym(ctx.q_rot.data(), ctx.q_sq8.data(), D);
    ...
    // SeedScore hoisted to NGT/NGTAQ/SearchContext.h (P0 PRISM Task 2).
    ...
    }
    {
        ...
        for (uint32_t cid_p : probe_clusters) {
            ...
            if (is_angular_ && !use_coarse) {
                ...
                NGT::NGTAQ::build_tier2_lut_fast_m(ctx.q_res.data(), M_PQ,
                                                    tier2_codebook_T_.data(),
    ...
    while (!cand_q.empty()) {
        ...
        if (dk_tracker.size() >= static_cast<size_t>(k_beam) &&
            ...
            if (use_coarse) {
                ...
                float t2ip = NGT::NGTAQ::tier2_adc_pq_m(ctx.t2_lut.data(), rec_x.tier2(), M_PQ);
        ...
        // NbrCand hoisted to NGT/NGTAQ/SearchContext.h (P0 PRISM Task 2).
